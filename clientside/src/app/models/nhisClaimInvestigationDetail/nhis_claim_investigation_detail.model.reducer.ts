...
import {initialNhisClaimInvestigationDetailCollectionState} from './nhis_claim_investigation_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisClaimInvestigationDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.CREATE_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				const createdModel = (action as modelAction.NhisClaimInvestigationDetailActionOK ).stateConfig.targetModel;
				...
				NhisClaimInvestigationDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.CREATE_NHIS_CLAIM_INVESTIGATION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.CREATE_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				const createdModels = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisClaimInvestigationDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.CREATE_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.DELETE_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				const deletedModelId = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.DELETE_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				const deletedModelIds = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.DELETE_NHIS_CLAIM_INVESTIGATION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.UPDATE_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				const updatedModel = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModel;
				...
				NhisClaimInvestigationDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.UPDATE_NHIS_CLAIM_INVESTIGATION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisClaimInvestigationDetailModelActionTypes.UPDATE_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				const updatedModels = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.UPDATE_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				let fetchedModel = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModel;
				...
				NhisClaimInvestigationDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisClaimInvestigationDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_NHIS_CLAIM_INVESTIGATION_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.NhisClaimInvestigationDetailActionOK).audits;
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_NHIS_CLAIM_INVESTIGATION_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisClaimInvestigationDetailActionOK).audits;
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_NHIS_CLAIM_INVESTIGATION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisClaimInvestigationDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_LAST_NHIS_CLAIM_INVESTIGATION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisClaimInvestigationDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.COUNT_NHIS_CLAIM_INVESTIGATION_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisClaimInvestigationDetailModelActionTypes.INITIALISE_NHIS_CLAIM_INVESTIGATION_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisClaimInvestigationDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisClaimInvestigationDetailCollectionState
					};
				}
