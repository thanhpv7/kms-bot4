...
import {initialCatalogueUOMCollectionState} from './catalogue_uom.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCatalogueUOMAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CatalogueUOMModelActionTypes.CREATE_CATALOGUE_UOM_OK:
				const createdModel = (action as modelAction.CatalogueUOMActionOK ).stateConfig.targetModel;
				...
				CatalogueUOMModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.CREATE_CATALOGUE_UOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueUOMModelActionTypes.CREATE_ALL_CATALOGUE_UOM_OK:
				const createdModels = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CatalogueUOMModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.CREATE_ALL_CATALOGUE_UOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueUOMModelActionTypes.DELETE_CATALOGUE_UOM_OK:
				const deletedModelId = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.DELETE_ALL_CATALOGUE_UOM_OK:
				const deletedModelIds = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.DELETE_CATALOGUE_UOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueUOMModelActionTypes.UPDATE_CATALOGUE_UOM_OK:
				const updatedModel = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModel;
				...
				CatalogueUOMModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.UPDATE_CATALOGUE_UOM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CatalogueUOMModelActionTypes.UPDATE_ALL_CATALOGUE_UOM_OK:
				const updatedModels = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.UPDATE_ALL_CATALOGUE_UOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueUOMModelActionTypes.FETCH_CATALOGUE_UOM_OK:
				let fetchedModel = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModel;
				...
				CatalogueUOMModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.FETCH_ALL_CATALOGUE_UOM_OK:
				const totalCountAfterFetchAll = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CatalogueUOMActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CatalogueUOMModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.FETCH_CATALOGUE_UOM_AUDIT_OK:
				const audits = (action as modelAction.CatalogueUOMActionOK).audits;
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.FETCH_CATALOGUE_UOM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CatalogueUOMActionOK).audits;
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.FETCH_CATALOGUE_UOM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CatalogueUOMModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.FETCH_LAST_CATALOGUE_UOM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CatalogueUOMActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CatalogueUOMModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.COUNT_CATALOGUE_UOMS_OK:
				const totalCountAfterCount = (action as modelAction.CatalogueUOMActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CatalogueUOMModelActionTypes.INITIALISE_CATALOGUE_UOM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CatalogueUOMActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CatalogueUOMActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCatalogueUOMCollectionState
					};
				}
