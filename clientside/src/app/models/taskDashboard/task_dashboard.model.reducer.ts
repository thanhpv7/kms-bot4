...
import {initialTaskDashboardCollectionState} from './task_dashboard.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTaskDashboardAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TaskDashboardModelActionTypes.CREATE_TASK_DASHBOARD_OK:
				const createdModel = (action as modelAction.TaskDashboardActionOK ).stateConfig.targetModel;
				...
				TaskDashboardModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.CREATE_TASK_DASHBOARD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TaskDashboardModelActionTypes.CREATE_ALL_TASK_DASHBOARD_OK:
				const createdModels = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TaskDashboardModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.CREATE_ALL_TASK_DASHBOARD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TaskDashboardModelActionTypes.DELETE_TASK_DASHBOARD_OK:
				const deletedModelId = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.DELETE_ALL_TASK_DASHBOARD_OK:
				const deletedModelIds = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.DELETE_TASK_DASHBOARD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TaskDashboardModelActionTypes.UPDATE_TASK_DASHBOARD_OK:
				const updatedModel = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModel;
				...
				TaskDashboardModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.UPDATE_TASK_DASHBOARD_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TaskDashboardModelActionTypes.UPDATE_ALL_TASK_DASHBOARD_OK:
				const updatedModels = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.UPDATE_ALL_TASK_DASHBOARD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TaskDashboardModelActionTypes.FETCH_TASK_DASHBOARD_OK:
				let fetchedModel = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModel;
				...
				TaskDashboardModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.FETCH_ALL_TASK_DASHBOARD_OK:
				const totalCountAfterFetchAll = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TaskDashboardActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TaskDashboardModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.FETCH_TASK_DASHBOARD_AUDIT_OK:
				const audits = (action as modelAction.TaskDashboardActionOK).audits;
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.FETCH_TASK_DASHBOARD_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TaskDashboardActionOK).audits;
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.FETCH_TASK_DASHBOARD_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TaskDashboardModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.FETCH_LAST_TASK_DASHBOARD_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TaskDashboardActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TaskDashboardModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.COUNT_TASK_DASHBOARDS_OK:
				const totalCountAfterCount = (action as modelAction.TaskDashboardActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TaskDashboardModelActionTypes.INITIALISE_TASK_DASHBOARD_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TaskDashboardActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TaskDashboardActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTaskDashboardCollectionState
					};
				}
