...
import {initialGoodsReceiveNoteItemCollectionState} from './goods_receive_note_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseGoodsReceiveNoteItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.GoodsReceiveNoteItemModelActionTypes.CREATE_GOODS_RECEIVE_NOTE_ITEM_OK:
				const createdModel = (action as modelAction.GoodsReceiveNoteItemActionOK ).stateConfig.targetModel;
				...
				GoodsReceiveNoteItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.CREATE_GOODS_RECEIVE_NOTE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.CREATE_ALL_GOODS_RECEIVE_NOTE_ITEM_OK:
				const createdModels = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					GoodsReceiveNoteItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.CREATE_ALL_GOODS_RECEIVE_NOTE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_ITEM_OK:
				const deletedModelId = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.DELETE_ALL_GOODS_RECEIVE_NOTE_ITEM_OK:
				const deletedModelIds = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE_ITEM_OK:
				const updatedModel = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModel;
				...
				GoodsReceiveNoteItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.GoodsReceiveNoteItemModelActionTypes.UPDATE_ALL_GOODS_RECEIVE_NOTE_ITEM_OK:
				const updatedModels = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.UPDATE_ALL_GOODS_RECEIVE_NOTE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_ITEM_OK:
				let fetchedModel = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModel;
				...
				GoodsReceiveNoteItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.FETCH_ALL_GOODS_RECEIVE_NOTE_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => GoodsReceiveNoteItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_ITEM_AUDIT_OK:
				const audits = (action as modelAction.GoodsReceiveNoteItemActionOK).audits;
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.GoodsReceiveNoteItemActionOK).audits;
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => GoodsReceiveNoteItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.FETCH_LAST_GOODS_RECEIVE_NOTE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => GoodsReceiveNoteItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.COUNT_GOODS_RECEIVE_NOTE_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.GoodsReceiveNoteItemModelActionTypes.INITIALISE_GOODS_RECEIVE_NOTE_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.GoodsReceiveNoteItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialGoodsReceiveNoteItemCollectionState
					};
				}
