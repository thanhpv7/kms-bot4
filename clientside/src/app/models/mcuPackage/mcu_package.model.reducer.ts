...
import {initialMcuPackageCollectionState} from './mcu_package.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMcuPackageAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.McuPackageModelActionTypes.CREATE_MCU_PACKAGE_OK:
				const createdModel = (action as modelAction.McuPackageActionOK ).stateConfig.targetModel;
				...
				McuPackageModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.McuPackageModelActionTypes.CREATE_MCU_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageModelActionTypes.CREATE_ALL_MCU_PACKAGE_OK:
				const createdModels = (action as modelAction.McuPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					McuPackageModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.McuPackageModelActionTypes.CREATE_ALL_MCU_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageModelActionTypes.DELETE_MCU_PACKAGE_OK:
				const deletedModelId = (action as modelAction.McuPackageActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.McuPackageModelActionTypes.DELETE_ALL_MCU_PACKAGE_OK:
				const deletedModelIds = (action as modelAction.McuPackageActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.McuPackageModelActionTypes.DELETE_MCU_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageModelActionTypes.UPDATE_MCU_PACKAGE_OK:
				const updatedModel = (action as modelAction.McuPackageActionOK).stateConfig.targetModel;
				...
				McuPackageModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageModelActionTypes.UPDATE_MCU_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.McuPackageModelActionTypes.UPDATE_ALL_MCU_PACKAGE_OK:
				const updatedModels = (action as modelAction.McuPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.McuPackageModelActionTypes.UPDATE_ALL_MCU_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageModelActionTypes.FETCH_MCU_PACKAGE_OK:
				let fetchedModel = (action as modelAction.McuPackageActionOK).stateConfig.targetModel;
				...
				McuPackageModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageModelActionTypes.FETCH_ALL_MCU_PACKAGE_OK:
				const totalCountAfterFetchAll = (action as modelAction.McuPackageActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.McuPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.McuPackageActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => McuPackageModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageModelActionTypes.FETCH_MCU_PACKAGE_AUDIT_OK:
				const audits = (action as modelAction.McuPackageActionOK).audits;
				...
				break;
			case modelAction.McuPackageModelActionTypes.FETCH_MCU_PACKAGE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.McuPackageActionOK).audits;
				...
				break;
			case modelAction.McuPackageModelActionTypes.FETCH_MCU_PACKAGE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => McuPackageModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageModelActionTypes.FETCH_LAST_MCU_PACKAGE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.McuPackageActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => McuPackageModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageModelActionTypes.COUNT_MCU_PACKAGES_OK:
				const totalCountAfterCount = (action as modelAction.McuPackageActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.McuPackageModelActionTypes.INITIALISE_MCU_PACKAGE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.McuPackageActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.McuPackageActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMcuPackageCollectionState
					};
				}
