...
import {initialPatientContactInfoCollectionState} from './patient_contact_info.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientContactInfoAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientContactInfoModelActionTypes.CREATE_PATIENT_CONTACT_INFO_OK:
				const createdModel = (action as modelAction.PatientContactInfoActionOK ).stateConfig.targetModel;
				...
				PatientContactInfoModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.CREATE_PATIENT_CONTACT_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientContactInfoModelActionTypes.CREATE_ALL_PATIENT_CONTACT_INFO_OK:
				const createdModels = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientContactInfoModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.CREATE_ALL_PATIENT_CONTACT_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientContactInfoModelActionTypes.DELETE_PATIENT_CONTACT_INFO_OK:
				const deletedModelId = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.DELETE_ALL_PATIENT_CONTACT_INFO_OK:
				const deletedModelIds = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.DELETE_PATIENT_CONTACT_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientContactInfoModelActionTypes.UPDATE_PATIENT_CONTACT_INFO_OK:
				const updatedModel = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModel;
				...
				PatientContactInfoModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.UPDATE_PATIENT_CONTACT_INFO_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientContactInfoModelActionTypes.UPDATE_ALL_PATIENT_CONTACT_INFO_OK:
				const updatedModels = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.UPDATE_ALL_PATIENT_CONTACT_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientContactInfoModelActionTypes.FETCH_PATIENT_CONTACT_INFO_OK:
				let fetchedModel = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModel;
				...
				PatientContactInfoModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.FETCH_ALL_PATIENT_CONTACT_INFO_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientContactInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientContactInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.FETCH_PATIENT_CONTACT_INFO_AUDIT_OK:
				const audits = (action as modelAction.PatientContactInfoActionOK).audits;
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.FETCH_PATIENT_CONTACT_INFO_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientContactInfoActionOK).audits;
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.FETCH_PATIENT_CONTACT_INFO_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientContactInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.FETCH_LAST_PATIENT_CONTACT_INFO_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientContactInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientContactInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.COUNT_PATIENT_CONTACT_INFOS_OK:
				const totalCountAfterCount = (action as modelAction.PatientContactInfoActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientContactInfoModelActionTypes.INITIALISE_PATIENT_CONTACT_INFO_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientContactInfoActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientContactInfoActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientContactInfoCollectionState
					};
				}
