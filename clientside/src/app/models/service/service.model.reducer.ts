...
import * as modelAction from './service.model.action';
import {ModelState} from '../model.state';
import {initialServiceCollectionState} from './service.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseServiceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ServiceModelActionTypes.CREATE_SERVICE_OK:
				const createdModel = (action as modelAction.ServiceActionOK ).stateConfig.targetModel;
				...
				ServiceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ServiceModelActionTypes.CREATE_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceModelActionTypes.CREATE_ALL_SERVICE_OK:
				const createdModels = (action as modelAction.ServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ServiceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ServiceModelActionTypes.CREATE_ALL_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceModelActionTypes.DELETE_SERVICE_OK:
				const deletedModelId = (action as modelAction.ServiceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceModelActionTypes.DELETE_ALL_SERVICE_OK:
				const deletedModelIds = (action as modelAction.ServiceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceModelActionTypes.DELETE_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceModelActionTypes.UPDATE_SERVICE_OK:
				const updatedModel = (action as modelAction.ServiceActionOK).stateConfig.targetModel;
				...
				ServiceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceModelActionTypes.UPDATE_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ServiceModelActionTypes.UPDATE_ALL_SERVICE_OK:
				const updatedModels = (action as modelAction.ServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceModelActionTypes.UPDATE_ALL_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceModelActionTypes.FETCH_SERVICE_OK:
				let fetchedModel = (action as modelAction.ServiceActionOK).stateConfig.targetModel;
				...
				ServiceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceModelActionTypes.FETCH_ALL_SERVICE_OK:
				const totalCountAfterFetchAll = (action as modelAction.ServiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ServiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ServiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceModelActionTypes.FETCH_SERVICE_AUDIT_OK:
				const audits = (action as modelAction.ServiceActionOK).audits;
				...
				break;
			case modelAction.ServiceModelActionTypes.FETCH_SERVICE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ServiceActionOK).audits;
				...
				break;
			case modelAction.ServiceModelActionTypes.FETCH_SERVICE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ServiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ServiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ServiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceModelActionTypes.FETCH_LAST_SERVICE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ServiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ServiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ServiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceModelActionTypes.COUNT_SERVICES_OK:
				const totalCountAfterCount = (action as modelAction.ServiceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ServiceModelActionTypes.INITIALISE_SERVICE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ServiceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ServiceActionOK).stateConfig.queryParams;
