 ...
 */
export class BpjsReferralSpecialistModel extends AbstractModel {
	/**
	...
	readonly className = 'BpjsReferralSpecialistModel';
	...
	get valueToDisplay(): string {
		...
		return BpjsReferralSpecialistModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BpjsReferralSpecialistModel.modelPropGroups;
	...
	static getProps(): ModelProperty[] {
		return super.getProps().concat([
			...
			{
				name: 'totalReferral',
				// % protected region % [Set displayName for Total Referral here] off begin
				displayName: 'Total Referral',
				// % protected region % [Set displayName for Total Referral here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Total Referral here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Total Referral here] end
				// % protected region % [Set isSensitive for Total Referral here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Total Referral here] end
				// % protected region % [Set readonly for Total Referral here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Total Referral here] end
				validators: [
					// % protected region % [Add other validators for Total Referral here] off begin
					// % protected region % [Add other validators for Total Referral here] end
				],
				// % protected region % [Add any additional model attribute properties for Total Referral here] off begin
				// % protected region % [Add any additional model attribute properties for Total Referral here] end
			},
			{
				name: 'percentage',
	 ...
	 */
	static deepParse(data: string | { [K in keyof BpjsReferralSpecialistModel]?: BpjsReferralSpecialistModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BpjsReferralSpecialistModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bpjsReferralSpecialistModel = new BpjsReferralSpecialistModel(data);`
	 *
	 * @param data The input data to be initialised as the BpjsReferralSpecialistModel,
	 *    it is expected as a JSON string or as a nullable BpjsReferralSpecialistModel.
	 */
	constructor(data?: string | Partial<BpjsReferralSpecialistModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BpjsReferralSpecialistModel>
				: data;

			this.specialistCode = json.specialistCode;
			this.specialistName = json.specialistName;
			this.capacity = json.capacity;
			this.totalReferral = json.totalReferral;
	...
	toJSON() {
		return {
			...
			totalReferral: this.totalReferral,
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BpjsReferralSpecialistModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BpjsReferralSpecialistModel {
		...
		if (updates.totalReferral) {
			newModelJson.totalReferral = updates.totalReferral;
		}
		...
		return new BpjsReferralSpecialistModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BpjsReferralSpecialistModel)) {
			return {};
		}
