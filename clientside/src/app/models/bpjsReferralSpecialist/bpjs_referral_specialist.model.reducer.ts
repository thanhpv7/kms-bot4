...
import {initialBpjsReferralSpecialistCollectionState} from './bpjs_referral_specialist.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsReferralSpecialistAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsReferralSpecialistModelActionTypes.CREATE_BPJS_REFERRAL_SPECIALIST_OK:
				const createdModel = (action as modelAction.BpjsReferralSpecialistActionOK ).stateConfig.targetModel;
				...
				BpjsReferralSpecialistModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.CREATE_BPJS_REFERRAL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.CREATE_ALL_BPJS_REFERRAL_SPECIALIST_OK:
				const createdModels = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsReferralSpecialistModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.CREATE_ALL_BPJS_REFERRAL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.DELETE_BPJS_REFERRAL_SPECIALIST_OK:
				const deletedModelId = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.DELETE_ALL_BPJS_REFERRAL_SPECIALIST_OK:
				const deletedModelIds = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.DELETE_BPJS_REFERRAL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.UPDATE_BPJS_REFERRAL_SPECIALIST_OK:
				const updatedModel = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModel;
				...
				BpjsReferralSpecialistModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.UPDATE_BPJS_REFERRAL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsReferralSpecialistModelActionTypes.UPDATE_ALL_BPJS_REFERRAL_SPECIALIST_OK:
				const updatedModels = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.UPDATE_ALL_BPJS_REFERRAL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_BPJS_REFERRAL_SPECIALIST_OK:
				let fetchedModel = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModel;
				...
				BpjsReferralSpecialistModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_ALL_BPJS_REFERRAL_SPECIALIST_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsReferralSpecialistModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_BPJS_REFERRAL_SPECIALIST_AUDIT_OK:
				const audits = (action as modelAction.BpjsReferralSpecialistActionOK).audits;
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_BPJS_REFERRAL_SPECIALIST_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsReferralSpecialistActionOK).audits;
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_BPJS_REFERRAL_SPECIALIST_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsReferralSpecialistModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_LAST_BPJS_REFERRAL_SPECIALIST_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsReferralSpecialistModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.COUNT_BPJS_REFERRAL_SPECIALISTS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsReferralSpecialistModelActionTypes.INITIALISE_BPJS_REFERRAL_SPECIALIST_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsReferralSpecialistActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsReferralSpecialistCollectionState
					};
				}
