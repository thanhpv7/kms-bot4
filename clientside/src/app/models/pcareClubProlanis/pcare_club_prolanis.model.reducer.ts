...
import {initialPcareClubProlanisCollectionState} from './pcare_club_prolanis.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareClubProlanisAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareClubProlanisModelActionTypes.CREATE_PCARE_CLUB_PROLANIS_OK:
				const createdModel = (action as modelAction.PcareClubProlanisActionOK ).stateConfig.targetModel;
				...
				PcareClubProlanisModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.CREATE_PCARE_CLUB_PROLANIS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareClubProlanisModelActionTypes.CREATE_ALL_PCARE_CLUB_PROLANIS_OK:
				const createdModels = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareClubProlanisModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.CREATE_ALL_PCARE_CLUB_PROLANIS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareClubProlanisModelActionTypes.DELETE_PCARE_CLUB_PROLANIS_OK:
				const deletedModelId = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.DELETE_ALL_PCARE_CLUB_PROLANIS_OK:
				const deletedModelIds = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.DELETE_PCARE_CLUB_PROLANIS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareClubProlanisModelActionTypes.UPDATE_PCARE_CLUB_PROLANIS_OK:
				const updatedModel = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModel;
				...
				PcareClubProlanisModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.UPDATE_PCARE_CLUB_PROLANIS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareClubProlanisModelActionTypes.UPDATE_ALL_PCARE_CLUB_PROLANIS_OK:
				const updatedModels = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.UPDATE_ALL_PCARE_CLUB_PROLANIS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareClubProlanisModelActionTypes.FETCH_PCARE_CLUB_PROLANIS_OK:
				let fetchedModel = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModel;
				...
				PcareClubProlanisModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.FETCH_ALL_PCARE_CLUB_PROLANIS_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareClubProlanisActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareClubProlanisModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.FETCH_PCARE_CLUB_PROLANIS_AUDIT_OK:
				const audits = (action as modelAction.PcareClubProlanisActionOK).audits;
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.FETCH_PCARE_CLUB_PROLANIS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareClubProlanisActionOK).audits;
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.FETCH_PCARE_CLUB_PROLANIS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareClubProlanisModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.FETCH_LAST_PCARE_CLUB_PROLANIS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareClubProlanisActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareClubProlanisModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.COUNT_PCARE_CLUB_PROLANISS_OK:
				const totalCountAfterCount = (action as modelAction.PcareClubProlanisActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareClubProlanisModelActionTypes.INITIALISE_PCARE_CLUB_PROLANIS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareClubProlanisActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareClubProlanisActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareClubProlanisCollectionState
					};
				}
