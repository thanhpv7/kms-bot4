...
import * as modelAction from './library.model.action';
import {ModelState} from '../model.state';
import {initialLibraryCollectionState} from './library.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseLibraryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.LibraryModelActionTypes.CREATE_LIBRARY_OK:
				const createdModel = (action as modelAction.LibraryActionOK ).stateConfig.targetModel;
				...
				LibraryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.LibraryModelActionTypes.CREATE_LIBRARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LibraryModelActionTypes.CREATE_ALL_LIBRARY_OK:
				const createdModels = (action as modelAction.LibraryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					LibraryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.LibraryModelActionTypes.CREATE_ALL_LIBRARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LibraryModelActionTypes.DELETE_LIBRARY_OK:
				const deletedModelId = (action as modelAction.LibraryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.LibraryModelActionTypes.DELETE_ALL_LIBRARY_OK:
				const deletedModelIds = (action as modelAction.LibraryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.LibraryModelActionTypes.DELETE_LIBRARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LibraryModelActionTypes.UPDATE_LIBRARY_OK:
				const updatedModel = (action as modelAction.LibraryActionOK).stateConfig.targetModel;
				...
				LibraryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LibraryModelActionTypes.UPDATE_LIBRARY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.LibraryModelActionTypes.UPDATE_ALL_LIBRARY_OK:
				const updatedModels = (action as modelAction.LibraryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.LibraryModelActionTypes.UPDATE_ALL_LIBRARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LibraryModelActionTypes.FETCH_LIBRARY_OK:
				let fetchedModel = (action as modelAction.LibraryActionOK).stateConfig.targetModel;
				...
				LibraryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LibraryModelActionTypes.FETCH_ALL_LIBRARY_OK:
				const totalCountAfterFetchAll = (action as modelAction.LibraryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.LibraryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.LibraryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => LibraryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LibraryModelActionTypes.FETCH_LIBRARY_AUDIT_OK:
				const audits = (action as modelAction.LibraryActionOK).audits;
				...
				break;
			case modelAction.LibraryModelActionTypes.FETCH_LIBRARY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.LibraryActionOK).audits;
				...
				break;
			case modelAction.LibraryModelActionTypes.FETCH_LIBRARY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.LibraryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.LibraryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.LibraryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => LibraryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LibraryModelActionTypes.FETCH_LAST_LIBRARY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.LibraryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.LibraryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.LibraryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => LibraryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LibraryModelActionTypes.COUNT_LIBRARYS_OK:
				const totalCountAfterCount = (action as modelAction.LibraryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.LibraryModelActionTypes.INITIALISE_LIBRARY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.LibraryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.LibraryActionOK).stateConfig.queryParams;
