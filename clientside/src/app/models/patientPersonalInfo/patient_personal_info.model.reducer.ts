...
import {initialPatientPersonalInfoCollectionState} from './patient_personal_info.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientPersonalInfoAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientPersonalInfoModelActionTypes.CREATE_PATIENT_PERSONAL_INFO_OK:
				const createdModel = (action as modelAction.PatientPersonalInfoActionOK ).stateConfig.targetModel;
				...
				PatientPersonalInfoModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.CREATE_PATIENT_PERSONAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.CREATE_ALL_PATIENT_PERSONAL_INFO_OK:
				const createdModels = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientPersonalInfoModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.CREATE_ALL_PATIENT_PERSONAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.DELETE_PATIENT_PERSONAL_INFO_OK:
				const deletedModelId = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.DELETE_ALL_PATIENT_PERSONAL_INFO_OK:
				const deletedModelIds = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.DELETE_PATIENT_PERSONAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.UPDATE_PATIENT_PERSONAL_INFO_OK:
				const updatedModel = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModel;
				...
				PatientPersonalInfoModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.UPDATE_PATIENT_PERSONAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientPersonalInfoModelActionTypes.UPDATE_ALL_PATIENT_PERSONAL_INFO_OK:
				const updatedModels = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.UPDATE_ALL_PATIENT_PERSONAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.FETCH_PATIENT_PERSONAL_INFO_OK:
				let fetchedModel = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModel;
				...
				PatientPersonalInfoModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.FETCH_ALL_PATIENT_PERSONAL_INFO_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientPersonalInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.FETCH_PATIENT_PERSONAL_INFO_AUDIT_OK:
				const audits = (action as modelAction.PatientPersonalInfoActionOK).audits;
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.FETCH_PATIENT_PERSONAL_INFO_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientPersonalInfoActionOK).audits;
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.FETCH_PATIENT_PERSONAL_INFO_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientPersonalInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.FETCH_LAST_PATIENT_PERSONAL_INFO_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientPersonalInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.COUNT_PATIENT_PERSONAL_INFOS_OK:
				const totalCountAfterCount = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientPersonalInfoModelActionTypes.INITIALISE_PATIENT_PERSONAL_INFO_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientPersonalInfoActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientPersonalInfoActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientPersonalInfoCollectionState
					};
				}
