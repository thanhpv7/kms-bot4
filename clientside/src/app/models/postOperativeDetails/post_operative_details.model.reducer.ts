...
import {initialPostOperativeDetailsCollectionState} from './post_operative_details.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePostOperativeDetailsAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PostOperativeDetailsModelActionTypes.CREATE_POST_OPERATIVE_DETAILS_OK:
				const createdModel = (action as modelAction.PostOperativeDetailsActionOK ).stateConfig.targetModel;
				...
				PostOperativeDetailsModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.CREATE_POST_OPERATIVE_DETAILS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.CREATE_ALL_POST_OPERATIVE_DETAILS_OK:
				const createdModels = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PostOperativeDetailsModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.CREATE_ALL_POST_OPERATIVE_DETAILS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.DELETE_POST_OPERATIVE_DETAILS_OK:
				const deletedModelId = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.DELETE_ALL_POST_OPERATIVE_DETAILS_OK:
				const deletedModelIds = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.DELETE_POST_OPERATIVE_DETAILS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.UPDATE_POST_OPERATIVE_DETAILS_OK:
				const updatedModel = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModel;
				...
				PostOperativeDetailsModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.UPDATE_POST_OPERATIVE_DETAILS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PostOperativeDetailsModelActionTypes.UPDATE_ALL_POST_OPERATIVE_DETAILS_OK:
				const updatedModels = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.UPDATE_ALL_POST_OPERATIVE_DETAILS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.FETCH_POST_OPERATIVE_DETAILS_OK:
				let fetchedModel = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModel;
				...
				PostOperativeDetailsModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.FETCH_ALL_POST_OPERATIVE_DETAILS_OK:
				const totalCountAfterFetchAll = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PostOperativeDetailsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.FETCH_POST_OPERATIVE_DETAILS_AUDIT_OK:
				const audits = (action as modelAction.PostOperativeDetailsActionOK).audits;
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.FETCH_POST_OPERATIVE_DETAILS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PostOperativeDetailsActionOK).audits;
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.FETCH_POST_OPERATIVE_DETAILS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PostOperativeDetailsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.FETCH_LAST_POST_OPERATIVE_DETAILS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PostOperativeDetailsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.COUNT_POST_OPERATIVE_DETAILSS_OK:
				const totalCountAfterCount = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PostOperativeDetailsModelActionTypes.INITIALISE_POST_OPERATIVE_DETAILS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PostOperativeDetailsActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PostOperativeDetailsActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPostOperativeDetailsCollectionState
					};
				}
