...
import {initialSystemAdminUserCollectionState} from './system_admin_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSystemAdminUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SystemAdminUserModelActionTypes.CREATE_SYSTEM_ADMIN_USER_OK:
				const createdModel = (action as modelAction.SystemAdminUserActionOK ).stateConfig.targetModel;
				...
				SystemAdminUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.CREATE_SYSTEM_ADMIN_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SystemAdminUserModelActionTypes.CREATE_ALL_SYSTEM_ADMIN_USER_OK:
				const createdModels = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SystemAdminUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.CREATE_ALL_SYSTEM_ADMIN_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SystemAdminUserModelActionTypes.DELETE_SYSTEM_ADMIN_USER_OK:
				const deletedModelId = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.DELETE_ALL_SYSTEM_ADMIN_USER_OK:
				const deletedModelIds = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.DELETE_SYSTEM_ADMIN_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SystemAdminUserModelActionTypes.UPDATE_SYSTEM_ADMIN_USER_OK:
				const updatedModel = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModel;
				...
				SystemAdminUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.UPDATE_SYSTEM_ADMIN_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SystemAdminUserModelActionTypes.UPDATE_ALL_SYSTEM_ADMIN_USER_OK:
				const updatedModels = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.UPDATE_ALL_SYSTEM_ADMIN_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SystemAdminUserModelActionTypes.FETCH_SYSTEM_ADMIN_USER_OK:
				let fetchedModel = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModel;
				...
				SystemAdminUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.FETCH_ALL_SYSTEM_ADMIN_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SystemAdminUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SystemAdminUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.FETCH_SYSTEM_ADMIN_USER_AUDIT_OK:
				const audits = (action as modelAction.SystemAdminUserActionOK).audits;
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.FETCH_SYSTEM_ADMIN_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SystemAdminUserActionOK).audits;
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.FETCH_SYSTEM_ADMIN_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SystemAdminUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.FETCH_LAST_SYSTEM_ADMIN_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SystemAdminUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SystemAdminUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.COUNT_SYSTEM_ADMIN_USERS_OK:
				const totalCountAfterCount = (action as modelAction.SystemAdminUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SystemAdminUserModelActionTypes.INITIALISE_SYSTEM_ADMIN_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SystemAdminUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SystemAdminUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialSystemAdminUserCollectionState
					};
				}
