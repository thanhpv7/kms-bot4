...
import * as modelAction from './staff.model.action';
import {ModelState} from '../model.state';
import {initialStaffCollectionState} from './staff.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseStaffAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.StaffModelActionTypes.CREATE_STAFF_OK:
				const createdModel = (action as modelAction.StaffActionOK ).stateConfig.targetModel;
				...
				StaffModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.StaffModelActionTypes.CREATE_STAFF_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StaffModelActionTypes.CREATE_ALL_STAFF_OK:
				const createdModels = (action as modelAction.StaffActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.StaffActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					StaffModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.StaffModelActionTypes.CREATE_ALL_STAFF_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StaffModelActionTypes.DELETE_STAFF_OK:
				const deletedModelId = (action as modelAction.StaffActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.StaffActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StaffModelActionTypes.DELETE_ALL_STAFF_OK:
				const deletedModelIds = (action as modelAction.StaffActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.StaffActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StaffModelActionTypes.DELETE_STAFF_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StaffModelActionTypes.UPDATE_STAFF_OK:
				const updatedModel = (action as modelAction.StaffActionOK).stateConfig.targetModel;
				...
				StaffModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StaffModelActionTypes.UPDATE_STAFF_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.StaffModelActionTypes.UPDATE_ALL_STAFF_OK:
				const updatedModels = (action as modelAction.StaffActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.StaffActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StaffModelActionTypes.UPDATE_ALL_STAFF_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StaffModelActionTypes.FETCH_STAFF_OK:
				let fetchedModel = (action as modelAction.StaffActionOK).stateConfig.targetModel;
				...
				StaffModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StaffModelActionTypes.FETCH_ALL_STAFF_OK:
				const totalCountAfterFetchAll = (action as modelAction.StaffActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.StaffActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.StaffActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.StaffActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => StaffModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StaffModelActionTypes.FETCH_STAFF_AUDIT_OK:
				const audits = (action as modelAction.StaffActionOK).audits;
				...
				break;
			case modelAction.StaffModelActionTypes.FETCH_STAFF_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.StaffActionOK).audits;
				...
				break;
			case modelAction.StaffModelActionTypes.FETCH_STAFF_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.StaffActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.StaffActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.StaffActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.StaffActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => StaffModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StaffModelActionTypes.FETCH_LAST_STAFF_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.StaffActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.StaffActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.StaffActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.StaffActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => StaffModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StaffModelActionTypes.COUNT_STAFFS_OK:
				const totalCountAfterCount = (action as modelAction.StaffActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.StaffModelActionTypes.INITIALISE_STAFF_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.StaffActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.StaffActionOK).stateConfig.queryParams;
