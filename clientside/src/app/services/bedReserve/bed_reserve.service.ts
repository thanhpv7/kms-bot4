...
import {BedReserveModel} from '../../models/bedReserve/bed_reserve.model';
import {BedReserveModelAudit} from '../../models/bedReserve/bed_reserve.model.state';
...
/**
 * Bed Reserve service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class BedReserveService extends AbstractHttpService<BedReserveModel, BedReserveModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the BedReserveProperties classFragment here] off begin
			fragment BedReserveProperties on BedReserve {
				...BedReserveBaseProperties
				notes
			}
			# % protected region % [Modify the BedReserveProperties classFragment here] end
			${super.fragments('BedReserve').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<BedReserveModel>> {
		...
		return  this.apollo.query<PassableStateConfig<BedReserveModel>>(
			{
				query: gql`
					query Count {
						totalCount: countBedReserves
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<BedReserveModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<BedReserveModel>> {
		...
		return this.apollo.query<PassableStateConfig<BedReserveModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						BedReserveById: bedReserveById(id: $id) {
							...BedReserveProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.BedReserveById as BedReserveModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<BedReserveModel>> {
		...
		return this.apollo.query<PassableStateConfig<BedReserveModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						bedReserves: bedReserves(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...BedReserveProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countBedReserves(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.bedReserves as BedReserveModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<BedReserveModel>> {
		...
		return this.apollo.query<PassableStateConfig<BedReserveModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						bedReserves: lastBedReserves(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...BedReserveProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: BedReserveModel, queryParams?: QueryParams): Observable<PassableStateConfig<BedReserveModel>> {
		...
		const successMessage = 'Created Bed Reserve entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($bedReserve: BedReserveInput!, $persistRelations: Boolean) {
						bedReserve: createBedReserve(bedReserve: $bedReserve, persistRelations: $persistRelations) {
							...BedReserveProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.bedReserve as BedReserveModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: BedReserveModel[], queryParams?: QueryParams): Observable<PassableStateConfig<BedReserveModel>> {
		...
		const successMessage = 'Created Multiple Bed Reserve entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($bedReserves: [BedReserveInput!]!) {
						bedReserve: createAllBedReserve(bedReserves: $bedReserves) {
							...BedReserveProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					bedReserves: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.bedReserve as BedReserveModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: BedReserveModel, queryParams?: QueryParams): Observable<PassableStateConfig<BedReserveModel>> {
		...
		const successMessage = 'Updated Bed Reserve entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($bedReserve: BedReserveInput!, $persistRelations: Boolean) {
						bedReserve: updateBedReserve(bedReserve: $bedReserve, persistRelations: $persistRelations) {
							...BedReserveProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: BedReserveModel[], queryParams?: QueryParams): Observable<PassableStateConfig<BedReserveModel>> {
		...
		const successMessage = 'Update Multiple Bed Reserve entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($bedReserves: [BedReserveInput!]!) {
						bedReserve: updateAllBedReserve(bedReserves: $bedReserves) {
							...BedReserveProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<BedReserveModel>> {
		...
		const successMessage = 'Deleted Bed Reserve entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($bedReserveId: ID!) {
						bedReserveId: deleteBedReserveById(bedReserveId: $bedReserveId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.bedReserveId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<BedReserveModel>> {
		...
		const successMessage = 'Deleted selected Bed Reserve entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($bedReserveIds: [ID!]!){
						deleteBedReserveExcludingIds(bedReserveIds: $bedReserveIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.bedReserveIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<BedReserveModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($bedReserveIds: [ID!]!) {
						bedReserveIds: deleteAllBedReserveByIds(bedReserveIds: $bedReserveIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<BedReserveModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<BedReserveModel>>(
			...
			map(result => {
				let audits: BedReserveModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new BedReserveModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<BedReserveModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<BedReserveModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($bedReserveId: ID!) {
						audits: bedReserveAuditsByEntityId(bedReserveId: $bedReserveId) {
							entity {
								...BedReserveProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<BedReserveModel>> {
		const url = environment.API_URL + '/api/bedreserve/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<BedReserveModel>> {
		const url = environment.API_URL + '/api/bedreserve/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<BedReserveModel>> {
		let apiUrl = environment.API_URL + '/api/bedreserve/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<BedReserveModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/bedreserve/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Bed Reserve entities')),
