...
import {BpjsClaimDataModelAudit} from '../../models/bpjsClaimData/bpjs_claim_data.model.state';
...
/**
 * BPJS Claim Data service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class BpjsClaimDataService extends AbstractHttpService<BpjsClaimDataModel, BpjsClaimDataModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the BpjsClaimDataProperties classFragment here] off begin
			fragment BpjsClaimDataProperties on BpjsClaimData {
				...BpjsClaimDataBaseProperties
				inacbgCode
				inacbgName
				submittedTariff
				approval
			...
			# % protected region % [Modify the BpjsClaimDataProperties classFragment here] end
			${super.fragments('BpjsClaimData').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		return  this.apollo.query<PassableStateConfig<BpjsClaimDataModel>>(
			{
				query: gql`
					query Count {
						totalCount: countBpjsClaimDatas
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<BpjsClaimDataModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		return this.apollo.query<PassableStateConfig<BpjsClaimDataModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						BpjsClaimDataById: bpjsClaimDataById(id: $id) {
							...BpjsClaimDataProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.BpjsClaimDataById as BpjsClaimDataModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		return this.apollo.query<PassableStateConfig<BpjsClaimDataModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						bpjsClaimDatas: bpjsClaimDatas(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...BpjsClaimDataProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countBpjsClaimDatas(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.bpjsClaimDatas as BpjsClaimDataModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		return this.apollo.query<PassableStateConfig<BpjsClaimDataModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						bpjsClaimDatas: lastBpjsClaimDatas(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...BpjsClaimDataProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: BpjsClaimDataModel, queryParams?: QueryParams): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		const successMessage = 'Created BPJS Claim Data entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($bpjsClaimData: BpjsClaimDataInput!, $persistRelations: Boolean) {
						bpjsClaimData: createBpjsClaimData(bpjsClaimData: $bpjsClaimData, persistRelations: $persistRelations) {
							...BpjsClaimDataProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.bpjsClaimData as BpjsClaimDataModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: BpjsClaimDataModel[], queryParams?: QueryParams): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		const successMessage = 'Created Multiple BPJS Claim Data entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($bpjsClaimDatas: [BpjsClaimDataInput!]!) {
						bpjsClaimData: createAllBpjsClaimData(bpjsClaimDatas: $bpjsClaimDatas) {
							...BpjsClaimDataProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					bpjsClaimDatas: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.bpjsClaimData as BpjsClaimDataModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: BpjsClaimDataModel, queryParams?: QueryParams): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		const successMessage = 'Updated BPJS Claim Data entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($bpjsClaimData: BpjsClaimDataInput!, $persistRelations: Boolean) {
						bpjsClaimData: updateBpjsClaimData(bpjsClaimData: $bpjsClaimData, persistRelations: $persistRelations) {
							...BpjsClaimDataProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: BpjsClaimDataModel[], queryParams?: QueryParams): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		const successMessage = 'Update Multiple BPJS Claim Data entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($bpjsClaimDatas: [BpjsClaimDataInput!]!) {
						bpjsClaimData: updateAllBpjsClaimData(bpjsClaimDatas: $bpjsClaimDatas) {
							...BpjsClaimDataProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		const successMessage = 'Deleted BPJS Claim Data entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($bpjsClaimDataId: ID!) {
						bpjsClaimDataId: deleteBpjsClaimDataById(bpjsClaimDataId: $bpjsClaimDataId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.bpjsClaimDataId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		const successMessage = 'Deleted selected BPJS Claim Data entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($bpjsClaimDataIds: [ID!]!){
						deleteBpjsClaimDataExcludingIds(bpjsClaimDataIds: $bpjsClaimDataIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.bpjsClaimDataIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($bpjsClaimDataIds: [ID!]!) {
						bpjsClaimDataIds: deleteAllBpjsClaimDataByIds(bpjsClaimDataIds: $bpjsClaimDataIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<BpjsClaimDataModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<BpjsClaimDataModel>>(
			...
			map(result => {
				let audits: BpjsClaimDataModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new BpjsClaimDataModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<BpjsClaimDataModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<BpjsClaimDataModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($bpjsClaimDataId: ID!) {
						audits: bpjsClaimDataAuditsByEntityId(bpjsClaimDataId: $bpjsClaimDataId) {
							entity {
								...BpjsClaimDataProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		const url = environment.API_URL + '/api/bpjsclaimdata/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		const url = environment.API_URL + '/api/bpjsclaimdata/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		let apiUrl = environment.API_URL + '/api/bpjsclaimdata/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<BpjsClaimDataModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/bpjsclaimdata/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' BPJS Claim Data entities')),
