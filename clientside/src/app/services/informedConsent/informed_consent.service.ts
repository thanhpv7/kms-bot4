...
import {InformedConsentModel} from '../../models/informedConsent/informed_consent.model';
import {InformedConsentModelAudit} from '../../models/informedConsent/informed_consent.model.state';
...
/**
 * Informed Consent service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class InformedConsentService extends AbstractHttpService<InformedConsentModel, InformedConsentModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the InformedConsentProperties classFragment here] off begin
			fragment InformedConsentProperties on InformedConsent {
				...InformedConsentBaseProperties
				patientFamilyGuardianName
				patientFamilyGuardianRelationship
				procedureToBePerformed
				consequenceOfProcedure
				consentOnProcedure
				consentDateTime
				informingStaffSignature
				consentingPartySignature
				consentingPartyType
				consentingPartyName
				witnessSignature1
				witnessName1
			...
			# % protected region % [Modify the InformedConsentProperties classFragment here] end
			${super.fragments('InformedConsent').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		return  this.apollo.query<PassableStateConfig<InformedConsentModel>>(
			{
				query: gql`
					query Count {
						totalCount: countInformedConsents
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<InformedConsentModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		return this.apollo.query<PassableStateConfig<InformedConsentModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						InformedConsentById: informedConsentById(id: $id) {
							...InformedConsentProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.InformedConsentById as InformedConsentModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		return this.apollo.query<PassableStateConfig<InformedConsentModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						informedConsents: informedConsents(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...InformedConsentProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countInformedConsents(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.informedConsents as InformedConsentModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		return this.apollo.query<PassableStateConfig<InformedConsentModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						informedConsents: lastInformedConsents(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...InformedConsentProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: InformedConsentModel, queryParams?: QueryParams): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		const successMessage = 'Created Informed Consent entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($informedConsent: InformedConsentInput!, $persistRelations: Boolean) {
						informedConsent: createInformedConsent(informedConsent: $informedConsent, persistRelations: $persistRelations) {
							...InformedConsentProperties,
				...
				variables: {
					informedConsent: {
						...entity.toJSON(),
						accompanyingStaff: entity.accompanyingStaff ? new StaffModel(entity.accompanyingStaff).toJSON() : undefined,
						...
						informingStaff: entity.informingStaff ? new StaffModel(entity.informingStaff).toJSON() : undefined,
					},
			...
			map(result => {
				...
				return {
					targetModel: result.data.informedConsent as InformedConsentModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: InformedConsentModel[], queryParams?: QueryParams): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		const successMessage = 'Created Multiple Informed Consent entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($informedConsents: [InformedConsentInput!]!) {
						informedConsent: createAllInformedConsent(informedConsents: $informedConsents) {
							...InformedConsentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					informedConsents: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.informedConsent as InformedConsentModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: InformedConsentModel, queryParams?: QueryParams): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		const successMessage = 'Updated Informed Consent entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($informedConsent: InformedConsentInput!, $persistRelations: Boolean) {
						informedConsent: updateInformedConsent(informedConsent: $informedConsent, persistRelations: $persistRelations) {
							...InformedConsentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: InformedConsentModel[], queryParams?: QueryParams): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		const successMessage = 'Update Multiple Informed Consent entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($informedConsents: [InformedConsentInput!]!) {
						informedConsent: updateAllInformedConsent(informedConsents: $informedConsents) {
							...InformedConsentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		const successMessage = 'Deleted Informed Consent entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($informedConsentId: ID!) {
						informedConsentId: deleteInformedConsentById(informedConsentId: $informedConsentId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.informedConsentId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		const successMessage = 'Deleted selected Informed Consent entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($informedConsentIds: [ID!]!){
						deleteInformedConsentExcludingIds(informedConsentIds: $informedConsentIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.informedConsentIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<InformedConsentModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($informedConsentIds: [ID!]!) {
						informedConsentIds: deleteAllInformedConsentByIds(informedConsentIds: $informedConsentIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<InformedConsentModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<InformedConsentModel>>(
			...
			map(result => {
				let audits: InformedConsentModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new InformedConsentModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<InformedConsentModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<InformedConsentModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($informedConsentId: ID!) {
						audits: informedConsentAuditsByEntityId(informedConsentId: $informedConsentId) {
							entity {
								...InformedConsentProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<InformedConsentModel>> {
		const url = environment.API_URL + '/api/informedconsent/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<InformedConsentModel>> {
		const url = environment.API_URL + '/api/informedconsent/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<InformedConsentModel>> {
		let apiUrl = environment.API_URL + '/api/informedconsent/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<InformedConsentModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/informedconsent/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Informed Consent entities')),
