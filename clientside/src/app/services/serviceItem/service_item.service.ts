...
import {ServiceItemModel} from '../../models/serviceItem/service_item.model';
import {ServiceItemModelAudit} from '../../models/serviceItem/service_item.model.state';
...
/**
 * Service Item service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class ServiceItemService extends AbstractHttpService<ServiceItemModel, ServiceItemModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the ServiceItemProperties classFragment here] off begin
			fragment ServiceItemProperties on ServiceItem {
				...ServiceItemBaseProperties
				serviceItemCode
				serviceItemName
				classification
				applyToAllService
			...
			# % protected region % [Modify the ServiceItemProperties classFragment here] end
			${super.fragments('ServiceItem').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		return  this.apollo.query<PassableStateConfig<ServiceItemModel>>(
			{
				query: gql`
					query Count {
						totalCount: countServiceItems
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<ServiceItemModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		return this.apollo.query<PassableStateConfig<ServiceItemModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						ServiceItemById: serviceItemById(id: $id) {
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.ServiceItemById as ServiceItemModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		return this.apollo.query<PassableStateConfig<ServiceItemModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						serviceItems: serviceItems(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...ServiceItemProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countServiceItems(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.serviceItems as ServiceItemModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		return this.apollo.query<PassableStateConfig<ServiceItemModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						serviceItems: lastServiceItems(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...ServiceItemProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: ServiceItemModel, queryParams?: QueryParams): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		const successMessage = 'Created Service Item entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($serviceItem: ServiceItemInput!, $persistRelations: Boolean) {
						serviceItem: createServiceItem(serviceItem: $serviceItem, persistRelations: $persistRelations) {
							...ServiceItemProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.serviceItem as ServiceItemModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: ServiceItemModel[], queryParams?: QueryParams): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		const successMessage = 'Created Multiple Service Item entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($serviceItems: [ServiceItemInput!]!) {
						serviceItem: createAllServiceItem(serviceItems: $serviceItems) {
							...ServiceItemProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					serviceItems: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.serviceItem as ServiceItemModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: ServiceItemModel, queryParams?: QueryParams): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		const successMessage = 'Updated Service Item entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($serviceItem: ServiceItemInput!, $persistRelations: Boolean) {
						serviceItem: updateServiceItem(serviceItem: $serviceItem, persistRelations: $persistRelations) {
							...ServiceItemProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: ServiceItemModel[], queryParams?: QueryParams): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		const successMessage = 'Update Multiple Service Item entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($serviceItems: [ServiceItemInput!]!) {
						serviceItem: updateAllServiceItem(serviceItems: $serviceItems) {
							...ServiceItemProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		const successMessage = 'Deleted Service Item entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($serviceItemId: ID!) {
						serviceItemId: deleteServiceItemById(serviceItemId: $serviceItemId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.serviceItemId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		const successMessage = 'Deleted selected Service Item entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($serviceItemIds: [ID!]!){
						deleteServiceItemExcludingIds(serviceItemIds: $serviceItemIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.serviceItemIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<ServiceItemModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($serviceItemIds: [ID!]!) {
						serviceItemIds: deleteAllServiceItemByIds(serviceItemIds: $serviceItemIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<ServiceItemModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<ServiceItemModel>>(
			...
			map(result => {
				let audits: ServiceItemModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new ServiceItemModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<ServiceItemModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<ServiceItemModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($serviceItemId: ID!) {
						audits: serviceItemAuditsByEntityId(serviceItemId: $serviceItemId) {
							entity {
								...ServiceItemProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<ServiceItemModel>> {
		const url = environment.API_URL + '/api/serviceitem/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<ServiceItemModel>> {
		const url = environment.API_URL + '/api/serviceitem/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<ServiceItemModel>> {
		let apiUrl = environment.API_URL + '/api/serviceitem/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<ServiceItemModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/serviceitem/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Service Item entities')),
