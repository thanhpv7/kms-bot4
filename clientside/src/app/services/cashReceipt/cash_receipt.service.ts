...
import {CashReceiptModel} from '../../models/cashReceipt/cash_receipt.model';
import {CashReceiptModelAudit} from '../../models/cashReceipt/cash_receipt.model.state';
...
/**
 * Cash Receipt service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class CashReceiptService extends AbstractHttpService<CashReceiptModel, CashReceiptModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the CashReceiptProperties classFragment here] off begin
			fragment CashReceiptProperties on CashReceipt {
				...CashReceiptBaseProperties
				receiptType
				receiptNumber
				paymentDate
				description
				...
				outstandingAmount
				receivedFrom
				patientOrCustomerName
			}
			# % protected region % [Modify the CashReceiptProperties classFragment here] end
			${super.fragments('CashReceipt').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		return  this.apollo.query<PassableStateConfig<CashReceiptModel>>(
			{
				query: gql`
					query Count {
						totalCount: countCashReceipts
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<CashReceiptModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashReceiptModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						CashReceiptById: cashReceiptById(id: $id) {
							...CashReceiptProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.CashReceiptById as CashReceiptModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashReceiptModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						cashReceipts: cashReceipts(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...CashReceiptProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countCashReceipts(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.cashReceipts as CashReceiptModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashReceiptModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						cashReceipts: lastCashReceipts(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...CashReceiptProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: CashReceiptModel, queryParams?: QueryParams): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		const successMessage = 'Created Cash Receipt entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($cashReceipt: CashReceiptInput!, $persistRelations: Boolean) {
						cashReceipt: createCashReceipt(cashReceipt: $cashReceipt, persistRelations: $persistRelations) {
							...CashReceiptProperties,
				...
				variables: {
					cashReceipt: {
						...entity.toJSON(),
						merchant: entity.merchant ? new MerchantModel(entity.merchant).toJSON() : undefined,
						...
						cashierStaff: entity.cashierStaff ? new StaffModel(entity.cashierStaff).toJSON() : undefined,
			...
			map(result => {
				...
				return {
					targetModel: result.data.cashReceipt as CashReceiptModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: CashReceiptModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		const successMessage = 'Created Multiple Cash Receipt entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($cashReceipts: [CashReceiptInput!]!) {
						cashReceipt: createAllCashReceipt(cashReceipts: $cashReceipts) {
							...CashReceiptProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					cashReceipts: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.cashReceipt as CashReceiptModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: CashReceiptModel, queryParams?: QueryParams): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		const successMessage = 'Updated Cash Receipt entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($cashReceipt: CashReceiptInput!, $persistRelations: Boolean) {
						cashReceipt: updateCashReceipt(cashReceipt: $cashReceipt, persistRelations: $persistRelations) {
							...CashReceiptProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: CashReceiptModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		const successMessage = 'Update Multiple Cash Receipt entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($cashReceipts: [CashReceiptInput!]!) {
						cashReceipt: updateAllCashReceipt(cashReceipts: $cashReceipts) {
							...CashReceiptProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		const successMessage = 'Deleted Cash Receipt entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($cashReceiptId: ID!) {
						cashReceiptId: deleteCashReceiptById(cashReceiptId: $cashReceiptId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.cashReceiptId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		const successMessage = 'Deleted selected Cash Receipt entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($cashReceiptIds: [ID!]!){
						deleteCashReceiptExcludingIds(cashReceiptIds: $cashReceiptIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.cashReceiptIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<CashReceiptModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($cashReceiptIds: [ID!]!) {
						cashReceiptIds: deleteAllCashReceiptByIds(cashReceiptIds: $cashReceiptIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<CashReceiptModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CashReceiptModel>>(
			...
			map(result => {
				let audits: CashReceiptModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new CashReceiptModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<CashReceiptModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CashReceiptModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($cashReceiptId: ID!) {
						audits: cashReceiptAuditsByEntityId(cashReceiptId: $cashReceiptId) {
							entity {
								...CashReceiptProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<CashReceiptModel>> {
		const url = environment.API_URL + '/api/cashreceipt/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<CashReceiptModel>> {
		const url = environment.API_URL + '/api/cashreceipt/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<CashReceiptModel>> {
		let apiUrl = environment.API_URL + '/api/cashreceipt/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<CashReceiptModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/cashreceipt/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Cash Receipt entities')),
