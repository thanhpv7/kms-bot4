...
import {RegistrationUserModel} from '../../models/registrationUser/registration_user.model';
import {RegistrationUserModelAudit} from '../../models/registrationUser/registration_user.model.state';
...
/**
 * Registration User service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class RegistrationUserService extends UserService<RegistrationUserModel, RegistrationUserModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the RegistrationUserProperties classFragment here] off begin
			fragment RegistrationUserProperties on RegistrationUser {
				...RegistrationUserUserProperties
			}
			# % protected region % [Modify the RegistrationUserProperties classFragment here] end
			${super.fragments('RegistrationUser').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		return  this.apollo.query<PassableStateConfig<RegistrationUserModel>>(
			{
				query: gql`
					query Count {
						totalCount: countRegistrationUsers
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<RegistrationUserModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<RegistrationUserModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						RegistrationUserById: registrationUserById(id: $id) {
							...RegistrationUserProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.RegistrationUserById as RegistrationUserModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<RegistrationUserModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						registrationUsers: registrationUsers(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...RegistrationUserProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countRegistrationUsers(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.registrationUsers as RegistrationUserModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<RegistrationUserModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						registrationUsers: lastRegistrationUsers(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...RegistrationUserProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: RegistrationUserModel, queryParams?: QueryParams): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		const successMessage = 'Created Registration User entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($registrationUser: RegistrationUserInput!, $persistRelations: Boolean) {
						registrationUser: createRegistrationUser(registrationUser: $registrationUser, persistRelations: $persistRelations) {
							...RegistrationUserProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.registrationUser as RegistrationUserModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: RegistrationUserModel[], queryParams?: QueryParams): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		const successMessage = 'Created Multiple Registration User entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($registrationUsers: [RegistrationUserInput!]!) {
						registrationUser: createAllRegistrationUser(registrationUsers: $registrationUsers) {
							...RegistrationUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					registrationUsers: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.registrationUser as RegistrationUserModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: RegistrationUserModel, queryParams?: QueryParams): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		const successMessage = 'Updated Registration User entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($registrationUser: RegistrationUserInput!, $persistRelations: Boolean) {
						registrationUser: updateRegistrationUser(registrationUser: $registrationUser, persistRelations: $persistRelations) {
							...RegistrationUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: RegistrationUserModel[], queryParams?: QueryParams): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		const successMessage = 'Update Multiple Registration User entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($registrationUsers: [RegistrationUserInput!]!) {
						registrationUser: updateAllRegistrationUser(registrationUsers: $registrationUsers) {
							...RegistrationUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		const successMessage = 'Deleted Registration User entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($registrationUserId: ID!) {
						registrationUserId: deleteRegistrationUserById(registrationUserId: $registrationUserId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.registrationUserId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		const successMessage = 'Deleted selected Registration User entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($registrationUserIds: [ID!]!){
						deleteRegistrationUserExcludingIds(registrationUserIds: $registrationUserIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.registrationUserIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<RegistrationUserModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($registrationUserIds: [ID!]!) {
						registrationUserIds: deleteAllRegistrationUserByIds(registrationUserIds: $registrationUserIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<RegistrationUserModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<RegistrationUserModel>>(
			...
			map(result => {
				let audits: RegistrationUserModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new RegistrationUserModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<RegistrationUserModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<RegistrationUserModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($registrationUserId: ID!) {
						audits: registrationUserAuditsByEntityId(registrationUserId: $registrationUserId) {
							entity {
								...RegistrationUserProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<RegistrationUserModel>> {
		const url = environment.API_URL + '/api/registrationuser/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<RegistrationUserModel>> {
		const url = environment.API_URL + '/api/registrationuser/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<RegistrationUserModel>> {
		let apiUrl = environment.API_URL + '/api/registrationuser/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<RegistrationUserModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/registrationuser/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Registration User entities')),
