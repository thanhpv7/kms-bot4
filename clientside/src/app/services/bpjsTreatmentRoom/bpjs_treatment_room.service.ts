...
import {BpjsTreatmentRoomModel} from '../../models/bpjsTreatmentRoom/bpjs_treatment_room.model';
import {BpjsTreatmentRoomModelAudit} from '../../models/bpjsTreatmentRoom/bpjs_treatment_room.model.state';
...
/**
 * BPJS Treatment Room service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class BpjsTreatmentRoomService extends AbstractHttpService<BpjsTreatmentRoomModel, BpjsTreatmentRoomModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the BpjsTreatmentRoomProperties classFragment here] off begin
			fragment BpjsTreatmentRoomProperties on BpjsTreatmentRoom {
				...BpjsTreatmentRoomBaseProperties
				kode
				nama
			}
			# % protected region % [Modify the BpjsTreatmentRoomProperties classFragment here] end
			${super.fragments('BpjsTreatmentRoom').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		return  this.apollo.query<PassableStateConfig<BpjsTreatmentRoomModel>>(
			{
				query: gql`
					query Count {
						totalCount: countBpjsTreatmentRooms
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		return this.apollo.query<PassableStateConfig<BpjsTreatmentRoomModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						BpjsTreatmentRoomById: bpjsTreatmentRoomById(id: $id) {
							...BpjsTreatmentRoomProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.BpjsTreatmentRoomById as BpjsTreatmentRoomModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		return this.apollo.query<PassableStateConfig<BpjsTreatmentRoomModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						bpjsTreatmentRooms: bpjsTreatmentRooms(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...BpjsTreatmentRoomProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countBpjsTreatmentRooms(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.bpjsTreatmentRooms as BpjsTreatmentRoomModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		return this.apollo.query<PassableStateConfig<BpjsTreatmentRoomModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						bpjsTreatmentRooms: lastBpjsTreatmentRooms(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...BpjsTreatmentRoomProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: BpjsTreatmentRoomModel, queryParams?: QueryParams): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		const successMessage = 'Created BPJS Treatment Room entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($bpjsTreatmentRoom: BpjsTreatmentRoomInput!, $persistRelations: Boolean) {
						bpjsTreatmentRoom: createBpjsTreatmentRoom(bpjsTreatmentRoom: $bpjsTreatmentRoom, persistRelations: $persistRelations) {
							...BpjsTreatmentRoomProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.bpjsTreatmentRoom as BpjsTreatmentRoomModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: BpjsTreatmentRoomModel[], queryParams?: QueryParams): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		const successMessage = 'Created Multiple BPJS Treatment Room entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($bpjsTreatmentRooms: [BpjsTreatmentRoomInput!]!) {
						bpjsTreatmentRoom: createAllBpjsTreatmentRoom(bpjsTreatmentRooms: $bpjsTreatmentRooms) {
							...BpjsTreatmentRoomProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					bpjsTreatmentRooms: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.bpjsTreatmentRoom as BpjsTreatmentRoomModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: BpjsTreatmentRoomModel, queryParams?: QueryParams): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		const successMessage = 'Updated BPJS Treatment Room entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($bpjsTreatmentRoom: BpjsTreatmentRoomInput!, $persistRelations: Boolean) {
						bpjsTreatmentRoom: updateBpjsTreatmentRoom(bpjsTreatmentRoom: $bpjsTreatmentRoom, persistRelations: $persistRelations) {
							...BpjsTreatmentRoomProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: BpjsTreatmentRoomModel[], queryParams?: QueryParams): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		const successMessage = 'Update Multiple BPJS Treatment Room entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($bpjsTreatmentRooms: [BpjsTreatmentRoomInput!]!) {
						bpjsTreatmentRoom: updateAllBpjsTreatmentRoom(bpjsTreatmentRooms: $bpjsTreatmentRooms) {
							...BpjsTreatmentRoomProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		const successMessage = 'Deleted BPJS Treatment Room entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($bpjsTreatmentRoomId: ID!) {
						bpjsTreatmentRoomId: deleteBpjsTreatmentRoomById(bpjsTreatmentRoomId: $bpjsTreatmentRoomId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.bpjsTreatmentRoomId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		const successMessage = 'Deleted selected BPJS Treatment Room entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($bpjsTreatmentRoomIds: [ID!]!){
						deleteBpjsTreatmentRoomExcludingIds(bpjsTreatmentRoomIds: $bpjsTreatmentRoomIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.bpjsTreatmentRoomIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($bpjsTreatmentRoomIds: [ID!]!) {
						bpjsTreatmentRoomIds: deleteAllBpjsTreatmentRoomByIds(bpjsTreatmentRoomIds: $bpjsTreatmentRoomIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<BpjsTreatmentRoomModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<BpjsTreatmentRoomModel>>(
			{
				query: gql`
					query GetAudits {
						audits: bpjsTreatmentRoomAudits {
							entity {
								...BpjsTreatmentRoomProperties
							}
							timestamp
			...
			map(result => {
				let audits: BpjsTreatmentRoomModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new BpjsTreatmentRoomModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<BpjsTreatmentRoomModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<BpjsTreatmentRoomModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($bpjsTreatmentRoomId: ID!) {
						audits: bpjsTreatmentRoomAuditsByEntityId(bpjsTreatmentRoomId: $bpjsTreatmentRoomId) {
							entity {
								...BpjsTreatmentRoomProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		const url = environment.API_URL + '/api/bpjstreatmentroom/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		const url = environment.API_URL + '/api/bpjstreatmentroom/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		let apiUrl = environment.API_URL + '/api/bpjstreatmentroom/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<BpjsTreatmentRoomModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/bpjstreatmentroom/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' BPJS Treatment Room entities')),
