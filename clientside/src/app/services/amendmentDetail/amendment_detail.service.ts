...
import {AmendmentDetailModel} from '../../models/amendmentDetail/amendment_detail.model';
import {AmendmentDetailModelAudit} from '../../models/amendmentDetail/amendment_detail.model.state';
...
/**
 * Amendment Detail service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class AmendmentDetailService extends AbstractHttpService<AmendmentDetailModel, AmendmentDetailModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the AmendmentDetailProperties classFragment here] off begin
			fragment AmendmentDetailProperties on AmendmentDetail {
				...AmendmentDetailBaseProperties
				date
				reason
				status
			}
			# % protected region % [Modify the AmendmentDetailProperties classFragment here] end
			${super.fragments('AmendmentDetail').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		return  this.apollo.query<PassableStateConfig<AmendmentDetailModel>>(
			{
				query: gql`
					query Count {
						totalCount: countAmendmentDetails
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<AmendmentDetailModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		return this.apollo.query<PassableStateConfig<AmendmentDetailModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						AmendmentDetailById: amendmentDetailById(id: $id) {
							...AmendmentDetailProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.AmendmentDetailById as AmendmentDetailModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		return this.apollo.query<PassableStateConfig<AmendmentDetailModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						amendmentDetails: amendmentDetails(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...AmendmentDetailProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countAmendmentDetails(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.amendmentDetails as AmendmentDetailModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		return this.apollo.query<PassableStateConfig<AmendmentDetailModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						amendmentDetails: lastAmendmentDetails(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...AmendmentDetailProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: AmendmentDetailModel, queryParams?: QueryParams): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		const successMessage = 'Created Amendment Detail entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($amendmentDetail: AmendmentDetailInput!, $persistRelations: Boolean) {
						amendmentDetail: createAmendmentDetail(amendmentDetail: $amendmentDetail, persistRelations: $persistRelations) {
							...AmendmentDetailProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.amendmentDetail as AmendmentDetailModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: AmendmentDetailModel[], queryParams?: QueryParams): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		const successMessage = 'Created Multiple Amendment Detail entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($amendmentDetails: [AmendmentDetailInput!]!) {
						amendmentDetail: createAllAmendmentDetail(amendmentDetails: $amendmentDetails) {
							...AmendmentDetailProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					amendmentDetails: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.amendmentDetail as AmendmentDetailModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: AmendmentDetailModel, queryParams?: QueryParams): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		const successMessage = 'Updated Amendment Detail entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($amendmentDetail: AmendmentDetailInput!, $persistRelations: Boolean) {
						amendmentDetail: updateAmendmentDetail(amendmentDetail: $amendmentDetail, persistRelations: $persistRelations) {
							...AmendmentDetailProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: AmendmentDetailModel[], queryParams?: QueryParams): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		const successMessage = 'Update Multiple Amendment Detail entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($amendmentDetails: [AmendmentDetailInput!]!) {
						amendmentDetail: updateAllAmendmentDetail(amendmentDetails: $amendmentDetails) {
							...AmendmentDetailProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		const successMessage = 'Deleted Amendment Detail entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($amendmentDetailId: ID!) {
						amendmentDetailId: deleteAmendmentDetailById(amendmentDetailId: $amendmentDetailId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.amendmentDetailId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		const successMessage = 'Deleted selected Amendment Detail entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($amendmentDetailIds: [ID!]!){
						deleteAmendmentDetailExcludingIds(amendmentDetailIds: $amendmentDetailIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.amendmentDetailIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<AmendmentDetailModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($amendmentDetailIds: [ID!]!) {
						amendmentDetailIds: deleteAllAmendmentDetailByIds(amendmentDetailIds: $amendmentDetailIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<AmendmentDetailModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<AmendmentDetailModel>>(
			...
			map(result => {
				let audits: AmendmentDetailModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new AmendmentDetailModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<AmendmentDetailModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<AmendmentDetailModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($amendmentDetailId: ID!) {
						audits: amendmentDetailAuditsByEntityId(amendmentDetailId: $amendmentDetailId) {
							entity {
								...AmendmentDetailProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<AmendmentDetailModel>> {
		const url = environment.API_URL + '/api/amendmentdetail/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<AmendmentDetailModel>> {
		const url = environment.API_URL + '/api/amendmentdetail/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<AmendmentDetailModel>> {
		let apiUrl = environment.API_URL + '/api/amendmentdetail/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<AmendmentDetailModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/amendmentdetail/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Amendment Detail entities')),
