...
import {RoleModelAudit} from '../../models/role/role.model.state';
...
/**
 * Role service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class RoleService extends AbstractHttpService<RoleModel, RoleModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the RoleProperties classFragment here] off begin
			fragment RoleProperties on Role {
				...RoleBaseProperties
				name
			}
			# % protected region % [Modify the RoleProperties classFragment here] end
			${super.fragments('Role').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<RoleModel>> {
		...
		return  this.apollo.query<PassableStateConfig<RoleModel>>(
			{
				query: gql`
					query Count {
						totalCount: countRoles
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<RoleModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<RoleModel>> {
		...
		return this.apollo.query<PassableStateConfig<RoleModel>>(
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.RoleById as RoleModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<RoleModel>> {
		...
		return this.apollo.query<PassableStateConfig<RoleModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						roles: roles(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...RoleProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countRoles(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.roles as RoleModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<RoleModel>> {
		...
		return this.apollo.query<PassableStateConfig<RoleModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						roles: lastRoles(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...RoleProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: RoleModel, queryParams?: QueryParams): Observable<PassableStateConfig<RoleModel>> {
		...
		const successMessage = 'Created Role entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($role: RoleInput!, $persistRelations: Boolean) {
						role: createRole(role: $role, persistRelations: $persistRelations) {
							...RoleProperties,
				...
				variables: {
					role: {
						...entity.toJSON(),
						privileges: entity.privileges ? entity.privileges?.map((model: PrivilegeModel) => new PrivilegeModel(model).toJSON()) : [],
						users: entity.users ? entity.users?.map((model: UserModel) => model.toJSON()) : [],
					},
			...
			map(result => {
				...
				return {
					targetModel: result.data.role as RoleModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: RoleModel[], queryParams?: QueryParams): Observable<PassableStateConfig<RoleModel>> {
		...
		const successMessage = 'Created Multiple Role entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($roles: [RoleInput!]!) {
						role: createAllRole(roles: $roles) {
							...RoleProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					roles: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.role as RoleModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: RoleModel, queryParams?: QueryParams): Observable<PassableStateConfig<RoleModel>> {
		...
		const successMessage = 'Updated Role entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($role: RoleInput!, $persistRelations: Boolean) {
						role: updateRole(role: $role, persistRelations: $persistRelations) {
							...RoleProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: RoleModel[], queryParams?: QueryParams): Observable<PassableStateConfig<RoleModel>> {
		...
		const successMessage = 'Update Multiple Role entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($roles: [RoleInput!]!) {
						role: updateAllRole(roles: $roles) {
							...RoleProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<RoleModel>> {
		...
		const successMessage = 'Deleted Role entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($roleId: ID!) {
						roleId: deleteRoleById(roleId: $roleId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.roleId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<RoleModel>> {
		...
		const successMessage = 'Deleted selected Role entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($roleIds: [ID!]!){
						deleteRoleExcludingIds(roleIds: $roleIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.roleIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<RoleModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($roleIds: [ID!]!) {
						roleIds: deleteAllRoleByIds(roleIds: $roleIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<RoleModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<RoleModel>>(
			...
			map(result => {
				let audits: RoleModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new RoleModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<RoleModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<RoleModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($roleId: ID!) {
						audits: roleAuditsByEntityId(roleId: $roleId) {
							entity {
								...RoleProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<RoleModel>> {
		const url = environment.API_URL + '/api/role/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<RoleModel>> {
		const url = environment.API_URL + '/api/role/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<RoleModel>> {
		let apiUrl = environment.API_URL + '/api/role/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<RoleModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/role/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Role entities')),
