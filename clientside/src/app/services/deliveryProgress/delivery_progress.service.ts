...
import {DeliveryProgressModelAudit} from '../../models/deliveryProgress/delivery_progress.model.state';
...
/**
 * Delivery Progress service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class DeliveryProgressService extends AbstractHttpService<DeliveryProgressModel, DeliveryProgressModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the DeliveryProgressProperties classFragment here] off begin
			fragment DeliveryProgressProperties on DeliveryProgress {
				...DeliveryProgressBaseProperties
				progressDateTime
				fhrPerMinute
				amnioticFluid
				moulding
			...
			# % protected region % [Modify the DeliveryProgressProperties classFragment here] end
			${super.fragments('DeliveryProgress').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		return  this.apollo.query<PassableStateConfig<DeliveryProgressModel>>(
			{
				query: gql`
					query Count {
						totalCount: countDeliveryProgresss
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<DeliveryProgressModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		return this.apollo.query<PassableStateConfig<DeliveryProgressModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						DeliveryProgressById: deliveryProgressById(id: $id) {
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.DeliveryProgressById as DeliveryProgressModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		return this.apollo.query<PassableStateConfig<DeliveryProgressModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						deliveryProgresss: deliveryProgresss(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...DeliveryProgressProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countDeliveryProgresss(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.deliveryProgresss as DeliveryProgressModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		return this.apollo.query<PassableStateConfig<DeliveryProgressModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						deliveryProgresss: lastDeliveryProgresss(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...DeliveryProgressProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: DeliveryProgressModel, queryParams?: QueryParams): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		const successMessage = 'Created Delivery Progress entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($deliveryProgress: DeliveryProgressInput!, $persistRelations: Boolean) {
						deliveryProgress: createDeliveryProgress(deliveryProgress: $deliveryProgress, persistRelations: $persistRelations) {
							...DeliveryProgressProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.deliveryProgress as DeliveryProgressModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: DeliveryProgressModel[], queryParams?: QueryParams): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		const successMessage = 'Created Multiple Delivery Progress entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($deliveryProgresss: [DeliveryProgressInput!]!) {
						deliveryProgress: createAllDeliveryProgress(deliveryProgresss: $deliveryProgresss) {
							...DeliveryProgressProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					deliveryProgresss: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.deliveryProgress as DeliveryProgressModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: DeliveryProgressModel, queryParams?: QueryParams): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		const successMessage = 'Updated Delivery Progress entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($deliveryProgress: DeliveryProgressInput!, $persistRelations: Boolean) {
						deliveryProgress: updateDeliveryProgress(deliveryProgress: $deliveryProgress, persistRelations: $persistRelations) {
							...DeliveryProgressProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: DeliveryProgressModel[], queryParams?: QueryParams): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		const successMessage = 'Update Multiple Delivery Progress entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($deliveryProgresss: [DeliveryProgressInput!]!) {
						deliveryProgress: updateAllDeliveryProgress(deliveryProgresss: $deliveryProgresss) {
							...DeliveryProgressProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		const successMessage = 'Deleted Delivery Progress entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($deliveryProgressId: ID!) {
						deliveryProgressId: deleteDeliveryProgressById(deliveryProgressId: $deliveryProgressId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.deliveryProgressId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		const successMessage = 'Deleted selected Delivery Progress entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($deliveryProgressIds: [ID!]!){
						deleteDeliveryProgressExcludingIds(deliveryProgressIds: $deliveryProgressIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.deliveryProgressIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<DeliveryProgressModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($deliveryProgressIds: [ID!]!) {
						deliveryProgressIds: deleteAllDeliveryProgressByIds(deliveryProgressIds: $deliveryProgressIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<DeliveryProgressModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<DeliveryProgressModel>>(
			...
			map(result => {
				let audits: DeliveryProgressModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new DeliveryProgressModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<DeliveryProgressModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<DeliveryProgressModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($deliveryProgressId: ID!) {
						audits: deliveryProgressAuditsByEntityId(deliveryProgressId: $deliveryProgressId) {
							entity {
								...DeliveryProgressProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<DeliveryProgressModel>> {
		const url = environment.API_URL + '/api/deliveryprogress/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<DeliveryProgressModel>> {
		const url = environment.API_URL + '/api/deliveryprogress/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<DeliveryProgressModel>> {
		let apiUrl = environment.API_URL + '/api/deliveryprogress/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<DeliveryProgressModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/deliveryprogress/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Delivery Progress entities')),
