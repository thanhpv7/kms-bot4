...
import {HealthFacilityModel} from '../../models/healthFacility/health_facility.model';
import {HealthFacilityModelAudit} from '../../models/healthFacility/health_facility.model.state';
...
/**
 * Health Facility service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class HealthFacilityService extends AbstractHttpService<HealthFacilityModel, HealthFacilityModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the HealthFacilityProperties classFragment here] off begin
			fragment HealthFacilityProperties on HealthFacility {
				...HealthFacilityBaseProperties
				healthFacilityID
				healthFacilityName
				address
				city
				...
				lastName
				phoneNumberPOC
				mobilePhoneNumberPOC
				emailPOC
				healthFacilityType
			}
			# % protected region % [Modify the HealthFacilityProperties classFragment here] end
			${super.fragments('HealthFacility').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		return  this.apollo.query<PassableStateConfig<HealthFacilityModel>>(
			{
				query: gql`
					query Count {
						totalCount: countHealthFacilitys
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<HealthFacilityModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		return this.apollo.query<PassableStateConfig<HealthFacilityModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						HealthFacilityById: healthFacilityById(id: $id) {
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.HealthFacilityById as HealthFacilityModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		return this.apollo.query<PassableStateConfig<HealthFacilityModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						healthFacilitys: healthFacilitys(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...HealthFacilityProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countHealthFacilitys(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.healthFacilitys as HealthFacilityModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		return this.apollo.query<PassableStateConfig<HealthFacilityModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						healthFacilitys: lastHealthFacilitys(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...HealthFacilityProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: HealthFacilityModel, queryParams?: QueryParams): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		const successMessage = 'Created Health Facility entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($healthFacility: HealthFacilityInput!, $persistRelations: Boolean) {
						healthFacility: createHealthFacility(healthFacility: $healthFacility, persistRelations: $persistRelations) {
							...HealthFacilityProperties,
				...
				variables: {
					healthFacility: {
						...entity.toJSON(),
						dismissalReferringFacility: entity.dismissalReferringFacility ? entity.dismissalReferringFacility?.map((model: RegistrationModel) => new RegistrationModel(model).toJSON()) : [],
						referringFacility: entity.referringFacility ? entity.referringFacility?.map((model: RegistrationModel) => new RegistrationModel(model).toJSON()) : [],
					},
			...
			map(result => {
				...
				return {
					targetModel: result.data.healthFacility as HealthFacilityModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: HealthFacilityModel[], queryParams?: QueryParams): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		const successMessage = 'Created Multiple Health Facility entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($healthFacilitys: [HealthFacilityInput!]!) {
						healthFacility: createAllHealthFacility(healthFacilitys: $healthFacilitys) {
							...HealthFacilityProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					healthFacilitys: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.healthFacility as HealthFacilityModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: HealthFacilityModel, queryParams?: QueryParams): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		const successMessage = 'Updated Health Facility entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($healthFacility: HealthFacilityInput!, $persistRelations: Boolean) {
						healthFacility: updateHealthFacility(healthFacility: $healthFacility, persistRelations: $persistRelations) {
							...HealthFacilityProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: HealthFacilityModel[], queryParams?: QueryParams): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		const successMessage = 'Update Multiple Health Facility entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($healthFacilitys: [HealthFacilityInput!]!) {
						healthFacility: updateAllHealthFacility(healthFacilitys: $healthFacilitys) {
							...HealthFacilityProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		const successMessage = 'Deleted Health Facility entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($healthFacilityId: ID!) {
						healthFacilityId: deleteHealthFacilityById(healthFacilityId: $healthFacilityId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.healthFacilityId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		const successMessage = 'Deleted selected Health Facility entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($healthFacilityIds: [ID!]!){
						deleteHealthFacilityExcludingIds(healthFacilityIds: $healthFacilityIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.healthFacilityIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<HealthFacilityModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($healthFacilityIds: [ID!]!) {
						healthFacilityIds: deleteAllHealthFacilityByIds(healthFacilityIds: $healthFacilityIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<HealthFacilityModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<HealthFacilityModel>>(
			...
			map(result => {
				let audits: HealthFacilityModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new HealthFacilityModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<HealthFacilityModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<HealthFacilityModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($healthFacilityId: ID!) {
						audits: healthFacilityAuditsByEntityId(healthFacilityId: $healthFacilityId) {
							entity {
								...HealthFacilityProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<HealthFacilityModel>> {
		const url = environment.API_URL + '/api/healthfacility/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<HealthFacilityModel>> {
		const url = environment.API_URL + '/api/healthfacility/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<HealthFacilityModel>> {
		let apiUrl = environment.API_URL + '/api/healthfacility/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<HealthFacilityModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/healthfacility/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Health Facility entities')),
