...
import {InvoicePaymentSelfPayingModel} from '../../models/invoicePaymentSelfPaying/invoice_payment_self_paying.model';
import {InvoicePaymentSelfPayingModelAudit} from '../../models/invoicePaymentSelfPaying/invoice_payment_self_paying.model.state';
...
/**
 * Invoice Payment Self Paying service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class InvoicePaymentSelfPayingService extends AbstractHttpService<InvoicePaymentSelfPayingModel, InvoicePaymentSelfPayingModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the InvoicePaymentSelfPayingProperties classFragment here] off begin
			fragment InvoicePaymentSelfPayingProperties on InvoicePaymentSelfPaying {
				...InvoicePaymentSelfPayingBaseProperties
				sponsorName
				sponsorAddress
				sponsorPhoneNo
				sponsorship
			}
			# % protected region % [Modify the InvoicePaymentSelfPayingProperties classFragment here] end
			${super.fragments('InvoicePaymentSelfPaying').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		return  this.apollo.query<PassableStateConfig<InvoicePaymentSelfPayingModel>>(
			{
				query: gql`
					query Count {
						totalCount: countInvoicePaymentSelfPayings
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		return this.apollo.query<PassableStateConfig<InvoicePaymentSelfPayingModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						InvoicePaymentSelfPayingById: invoicePaymentSelfPayingById(id: $id) {
							...InvoicePaymentSelfPayingProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.InvoicePaymentSelfPayingById as InvoicePaymentSelfPayingModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		return this.apollo.query<PassableStateConfig<InvoicePaymentSelfPayingModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						invoicePaymentSelfPayings: invoicePaymentSelfPayings(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...InvoicePaymentSelfPayingProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countInvoicePaymentSelfPayings(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.invoicePaymentSelfPayings as InvoicePaymentSelfPayingModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		return this.apollo.query<PassableStateConfig<InvoicePaymentSelfPayingModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						invoicePaymentSelfPayings: lastInvoicePaymentSelfPayings(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...InvoicePaymentSelfPayingProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: InvoicePaymentSelfPayingModel, queryParams?: QueryParams): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		const successMessage = 'Created Invoice Payment Self Paying entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($invoicePaymentSelfPaying: InvoicePaymentSelfPayingInput!, $persistRelations: Boolean) {
						invoicePaymentSelfPaying: createInvoicePaymentSelfPaying(invoicePaymentSelfPaying: $invoicePaymentSelfPaying, persistRelations: $persistRelations) {
							...InvoicePaymentSelfPayingProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.invoicePaymentSelfPaying as InvoicePaymentSelfPayingModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: InvoicePaymentSelfPayingModel[], queryParams?: QueryParams): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		const successMessage = 'Created Multiple Invoice Payment Self Paying entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($invoicePaymentSelfPayings: [InvoicePaymentSelfPayingInput!]!) {
						invoicePaymentSelfPaying: createAllInvoicePaymentSelfPaying(invoicePaymentSelfPayings: $invoicePaymentSelfPayings) {
							...InvoicePaymentSelfPayingProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					invoicePaymentSelfPayings: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.invoicePaymentSelfPaying as InvoicePaymentSelfPayingModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: InvoicePaymentSelfPayingModel, queryParams?: QueryParams): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		const successMessage = 'Updated Invoice Payment Self Paying entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($invoicePaymentSelfPaying: InvoicePaymentSelfPayingInput!, $persistRelations: Boolean) {
						invoicePaymentSelfPaying: updateInvoicePaymentSelfPaying(invoicePaymentSelfPaying: $invoicePaymentSelfPaying, persistRelations: $persistRelations) {
							...InvoicePaymentSelfPayingProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: InvoicePaymentSelfPayingModel[], queryParams?: QueryParams): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		const successMessage = 'Update Multiple Invoice Payment Self Paying entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($invoicePaymentSelfPayings: [InvoicePaymentSelfPayingInput!]!) {
						invoicePaymentSelfPaying: updateAllInvoicePaymentSelfPaying(invoicePaymentSelfPayings: $invoicePaymentSelfPayings) {
							...InvoicePaymentSelfPayingProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		const successMessage = 'Deleted Invoice Payment Self Paying entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($invoicePaymentSelfPayingId: ID!) {
						invoicePaymentSelfPayingId: deleteInvoicePaymentSelfPayingById(invoicePaymentSelfPayingId: $invoicePaymentSelfPayingId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.invoicePaymentSelfPayingId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		const successMessage = 'Deleted selected Invoice Payment Self Paying entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($invoicePaymentSelfPayingIds: [ID!]!){
						deleteInvoicePaymentSelfPayingExcludingIds(invoicePaymentSelfPayingIds: $invoicePaymentSelfPayingIds)
				...
				`,
				variables: {
					invoicePaymentSelfPayingIds: ids,
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.invoicePaymentSelfPayingIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($invoicePaymentSelfPayingIds: [ID!]!) {
						invoicePaymentSelfPayingIds: deleteAllInvoicePaymentSelfPayingByIds(invoicePaymentSelfPayingIds: $invoicePaymentSelfPayingIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<InvoicePaymentSelfPayingModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<InvoicePaymentSelfPayingModel>>(
			{
				query: gql`
					query GetAudits {
						audits: invoicePaymentSelfPayingAudits {
							entity {
								...InvoicePaymentSelfPayingProperties
							}
							timestamp
			...
			map(result => {
				let audits: InvoicePaymentSelfPayingModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new InvoicePaymentSelfPayingModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<InvoicePaymentSelfPayingModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<InvoicePaymentSelfPayingModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($invoicePaymentSelfPayingId: ID!) {
						audits: invoicePaymentSelfPayingAuditsByEntityId(invoicePaymentSelfPayingId: $invoicePaymentSelfPayingId) {
							entity {
								...InvoicePaymentSelfPayingProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		const url = environment.API_URL + '/api/invoicepaymentselfpaying/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		const url = environment.API_URL + '/api/invoicepaymentselfpaying/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		let apiUrl = environment.API_URL + '/api/invoicepaymentselfpaying/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<InvoicePaymentSelfPayingModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/invoicepaymentselfpaying/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Invoice Payment Self Paying entities')),
