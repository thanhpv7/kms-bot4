...
import {NhisClaimModel} from '../../models/nhisClaim/nhis_claim.model';
import {NhisClaimModelAudit} from '../../models/nhisClaim/nhis_claim.model.state';
...
/**
 * NHIS Claim service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class NhisClaimService extends AbstractHttpService<NhisClaimModel, NhisClaimModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the NhisClaimProperties classFragment here] off begin
			fragment NhisClaimProperties on NhisClaim {
				...NhisClaimBaseProperties
				systemClaimID
				refInvoiceNo
				formNo
				claimID
				...
				typeOfAttendance
				firstVisitAdmission
				secondVisitDischarge
				thirdVisit
				fourthVisit
				physicianName
				physicianCode
			}
			# % protected region % [Modify the NhisClaimProperties classFragment here] end
			${super.fragments('NhisClaim').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		return  this.apollo.query<PassableStateConfig<NhisClaimModel>>(
			{
				query: gql`
					query Count {
						totalCount: countNhisClaims
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<NhisClaimModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		return this.apollo.query<PassableStateConfig<NhisClaimModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						NhisClaimById: nhisClaimById(id: $id) {
							...NhisClaimProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.NhisClaimById as NhisClaimModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		return this.apollo.query<PassableStateConfig<NhisClaimModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						nhisClaims: nhisClaims(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...NhisClaimProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countNhisClaims(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.nhisClaims as NhisClaimModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		return this.apollo.query<PassableStateConfig<NhisClaimModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						nhisClaims: lastNhisClaims(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...NhisClaimProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: NhisClaimModel, queryParams?: QueryParams): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		const successMessage = 'Created NHIS Claim entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($nhisClaim: NhisClaimInput!, $persistRelations: Boolean) {
						nhisClaim: createNhisClaim(nhisClaim: $nhisClaim, persistRelations: $persistRelations) {
							...NhisClaimProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.nhisClaim as NhisClaimModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: NhisClaimModel[], queryParams?: QueryParams): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		const successMessage = 'Created Multiple NHIS Claim entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($nhisClaims: [NhisClaimInput!]!) {
						nhisClaim: createAllNhisClaim(nhisClaims: $nhisClaims) {
							...NhisClaimProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					nhisClaims: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.nhisClaim as NhisClaimModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: NhisClaimModel, queryParams?: QueryParams): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		const successMessage = 'Updated NHIS Claim entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($nhisClaim: NhisClaimInput!, $persistRelations: Boolean) {
						nhisClaim: updateNhisClaim(nhisClaim: $nhisClaim, persistRelations: $persistRelations) {
							...NhisClaimProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: NhisClaimModel[], queryParams?: QueryParams): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		const successMessage = 'Update Multiple NHIS Claim entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($nhisClaims: [NhisClaimInput!]!) {
						nhisClaim: updateAllNhisClaim(nhisClaims: $nhisClaims) {
							...NhisClaimProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		const successMessage = 'Deleted NHIS Claim entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($nhisClaimId: ID!) {
						nhisClaimId: deleteNhisClaimById(nhisClaimId: $nhisClaimId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.nhisClaimId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		const successMessage = 'Deleted selected NHIS Claim entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($nhisClaimIds: [ID!]!){
						deleteNhisClaimExcludingIds(nhisClaimIds: $nhisClaimIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.nhisClaimIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<NhisClaimModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($nhisClaimIds: [ID!]!) {
						nhisClaimIds: deleteAllNhisClaimByIds(nhisClaimIds: $nhisClaimIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<NhisClaimModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<NhisClaimModel>>(
			...
			map(result => {
				let audits: NhisClaimModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new NhisClaimModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<NhisClaimModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<NhisClaimModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($nhisClaimId: ID!) {
						audits: nhisClaimAuditsByEntityId(nhisClaimId: $nhisClaimId) {
							entity {
								...NhisClaimProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<NhisClaimModel>> {
		const url = environment.API_URL + '/api/nhisclaim/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<NhisClaimModel>> {
		const url = environment.API_URL + '/api/nhisclaim/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<NhisClaimModel>> {
		let apiUrl = environment.API_URL + '/api/nhisclaim/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<NhisClaimModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/nhisclaim/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' NHIS Claim entities')),
