...
import {CashierUserModel} from '../../models/cashierUser/cashier_user.model';
import {CashierUserModelAudit} from '../../models/cashierUser/cashier_user.model.state';
...
/**
 * Cashier User service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class CashierUserService extends UserService<CashierUserModel, CashierUserModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the CashierUserProperties classFragment here] off begin
			fragment CashierUserProperties on CashierUser {
				...CashierUserUserProperties
			}
			# % protected region % [Modify the CashierUserProperties classFragment here] end
			${super.fragments('CashierUser').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<CashierUserModel>> {
		...
		return  this.apollo.query<PassableStateConfig<CashierUserModel>>(
			{
				query: gql`
					query Count {
						totalCount: countCashierUsers
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<CashierUserModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<CashierUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashierUserModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						CashierUserById: cashierUserById(id: $id) {
							...CashierUserProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.CashierUserById as CashierUserModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CashierUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashierUserModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						cashierUsers: cashierUsers(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...CashierUserProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countCashierUsers(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.cashierUsers as CashierUserModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CashierUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashierUserModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						cashierUsers: lastCashierUsers(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...CashierUserProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: CashierUserModel, queryParams?: QueryParams): Observable<PassableStateConfig<CashierUserModel>> {
		...
		const successMessage = 'Created Cashier User entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($cashierUser: CashierUserInput!, $persistRelations: Boolean) {
						cashierUser: createCashierUser(cashierUser: $cashierUser, persistRelations: $persistRelations) {
							...CashierUserProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.cashierUser as CashierUserModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: CashierUserModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CashierUserModel>> {
		...
		const successMessage = 'Created Multiple Cashier User entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($cashierUsers: [CashierUserInput!]!) {
						cashierUser: createAllCashierUser(cashierUsers: $cashierUsers) {
							...CashierUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					cashierUsers: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.cashierUser as CashierUserModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: CashierUserModel, queryParams?: QueryParams): Observable<PassableStateConfig<CashierUserModel>> {
		...
		const successMessage = 'Updated Cashier User entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($cashierUser: CashierUserInput!, $persistRelations: Boolean) {
						cashierUser: updateCashierUser(cashierUser: $cashierUser, persistRelations: $persistRelations) {
							...CashierUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: CashierUserModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CashierUserModel>> {
		...
		const successMessage = 'Update Multiple Cashier User entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($cashierUsers: [CashierUserInput!]!) {
						cashierUser: updateAllCashierUser(cashierUsers: $cashierUsers) {
							...CashierUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<CashierUserModel>> {
		...
		const successMessage = 'Deleted Cashier User entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($cashierUserId: ID!) {
						cashierUserId: deleteCashierUserById(cashierUserId: $cashierUserId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.cashierUserId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<CashierUserModel>> {
		...
		const successMessage = 'Deleted selected Cashier User entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($cashierUserIds: [ID!]!){
						deleteCashierUserExcludingIds(cashierUserIds: $cashierUserIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.cashierUserIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<CashierUserModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($cashierUserIds: [ID!]!) {
						cashierUserIds: deleteAllCashierUserByIds(cashierUserIds: $cashierUserIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<CashierUserModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CashierUserModel>>(
			...
			map(result => {
				let audits: CashierUserModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new CashierUserModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<CashierUserModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CashierUserModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($cashierUserId: ID!) {
						audits: cashierUserAuditsByEntityId(cashierUserId: $cashierUserId) {
							entity {
								...CashierUserProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<CashierUserModel>> {
		const url = environment.API_URL + '/api/cashieruser/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<CashierUserModel>> {
		const url = environment.API_URL + '/api/cashieruser/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<CashierUserModel>> {
		let apiUrl = environment.API_URL + '/api/cashieruser/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<CashierUserModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/cashieruser/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Cashier User entities')),
