...
import {JournalModel} from '../../models/journal/journal.model';
import {JournalModelAudit} from '../../models/journal/journal.model.state';
...
/**
 * Journal service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class JournalService extends AbstractHttpService<JournalModel, JournalModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the JournalProperties classFragment here] off begin
			fragment JournalProperties on Journal {
			...
			# % protected region % [Modify the JournalProperties classFragment here] end
			${super.fragments('Journal').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<JournalModel>> {
		...
		return  this.apollo.query<PassableStateConfig<JournalModel>>(
			{
				query: gql`
					query Count {
						totalCount: countJournals
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<JournalModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<JournalModel>> {
		...
		return this.apollo.query<PassableStateConfig<JournalModel>>(
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.JournalById as JournalModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<JournalModel>> {
		...
		return this.apollo.query<PassableStateConfig<JournalModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						journals: journals(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...JournalProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countJournals(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.journals as JournalModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<JournalModel>> {
		...
		return this.apollo.query<PassableStateConfig<JournalModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						journals: lastJournals(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...JournalProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: JournalModel, queryParams?: QueryParams): Observable<PassableStateConfig<JournalModel>> {
		...
		const successMessage = 'Created Journal entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($journal: JournalInput!, $persistRelations: Boolean) {
						journal: createJournal(journal: $journal, persistRelations: $persistRelations) {
							...JournalProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.journal as JournalModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: JournalModel[], queryParams?: QueryParams): Observable<PassableStateConfig<JournalModel>> {
		...
		const successMessage = 'Created Multiple Journal entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($journals: [JournalInput!]!) {
						journal: createAllJournal(journals: $journals) {
							...JournalProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					journals: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.journal as JournalModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: JournalModel, queryParams?: QueryParams): Observable<PassableStateConfig<JournalModel>> {
		...
		const successMessage = 'Updated Journal entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($journal: JournalInput!, $persistRelations: Boolean) {
						journal: updateJournal(journal: $journal, persistRelations: $persistRelations) {
							...JournalProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: JournalModel[], queryParams?: QueryParams): Observable<PassableStateConfig<JournalModel>> {
		...
		const successMessage = 'Update Multiple Journal entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($journals: [JournalInput!]!) {
						journal: updateAllJournal(journals: $journals) {
							...JournalProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<JournalModel>> {
		...
		const successMessage = 'Deleted Journal entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($journalId: ID!) {
						journalId: deleteJournalById(journalId: $journalId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.journalId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<JournalModel>> {
		...
		const successMessage = 'Deleted selected Journal entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($journalIds: [ID!]!){
						deleteJournalExcludingIds(journalIds: $journalIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.journalIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<JournalModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($journalIds: [ID!]!) {
						journalIds: deleteAllJournalByIds(journalIds: $journalIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<JournalModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<JournalModel>>(
			...
			map(result => {
				let audits: JournalModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new JournalModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<JournalModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<JournalModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($journalId: ID!) {
						audits: journalAuditsByEntityId(journalId: $journalId) {
							entity {
								...JournalProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<JournalModel>> {
		const url = environment.API_URL + '/api/journal/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<JournalModel>> {
		const url = environment.API_URL + '/api/journal/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<JournalModel>> {
		let apiUrl = environment.API_URL + '/api/journal/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<JournalModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/journal/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Journal entities')),
