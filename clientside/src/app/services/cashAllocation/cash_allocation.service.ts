...
import {CashAllocationModelAudit} from '../../models/cashAllocation/cash_allocation.model.state';
...
/**
 * Cash Allocation service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class CashAllocationService extends AbstractHttpService<CashAllocationModel, CashAllocationModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the CashAllocationProperties classFragment here] off begin
			fragment CashAllocationProperties on CashAllocation {
				...CashAllocationBaseProperties
				cashType
				allocatedDate
				sourceDate
				cashNumber
			...
			# % protected region % [Modify the CashAllocationProperties classFragment here] end
			${super.fragments('CashAllocation').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		return  this.apollo.query<PassableStateConfig<CashAllocationModel>>(
			{
				query: gql`
					query Count {
						totalCount: countCashAllocations
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<CashAllocationModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashAllocationModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						CashAllocationById: cashAllocationById(id: $id) {
							...CashAllocationProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.CashAllocationById as CashAllocationModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashAllocationModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						cashAllocations: cashAllocations(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...CashAllocationProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countCashAllocations(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.cashAllocations as CashAllocationModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		return this.apollo.query<PassableStateConfig<CashAllocationModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						cashAllocations: lastCashAllocations(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...CashAllocationProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: CashAllocationModel, queryParams?: QueryParams): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		const successMessage = 'Created Cash Allocation entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($cashAllocation: CashAllocationInput!, $persistRelations: Boolean) {
						cashAllocation: createCashAllocation(cashAllocation: $cashAllocation, persistRelations: $persistRelations) {
							...CashAllocationProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.cashAllocation as CashAllocationModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: CashAllocationModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		const successMessage = 'Created Multiple Cash Allocation entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($cashAllocations: [CashAllocationInput!]!) {
						cashAllocation: createAllCashAllocation(cashAllocations: $cashAllocations) {
							...CashAllocationProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					cashAllocations: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.cashAllocation as CashAllocationModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: CashAllocationModel, queryParams?: QueryParams): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		const successMessage = 'Updated Cash Allocation entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($cashAllocation: CashAllocationInput!, $persistRelations: Boolean) {
						cashAllocation: updateCashAllocation(cashAllocation: $cashAllocation, persistRelations: $persistRelations) {
							...CashAllocationProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: CashAllocationModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		const successMessage = 'Update Multiple Cash Allocation entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($cashAllocations: [CashAllocationInput!]!) {
						cashAllocation: updateAllCashAllocation(cashAllocations: $cashAllocations) {
							...CashAllocationProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		const successMessage = 'Deleted Cash Allocation entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($cashAllocationId: ID!) {
						cashAllocationId: deleteCashAllocationById(cashAllocationId: $cashAllocationId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.cashAllocationId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		const successMessage = 'Deleted selected Cash Allocation entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($cashAllocationIds: [ID!]!){
						deleteCashAllocationExcludingIds(cashAllocationIds: $cashAllocationIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.cashAllocationIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<CashAllocationModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($cashAllocationIds: [ID!]!) {
						cashAllocationIds: deleteAllCashAllocationByIds(cashAllocationIds: $cashAllocationIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<CashAllocationModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CashAllocationModel>>(
			...
			map(result => {
				let audits: CashAllocationModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new CashAllocationModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<CashAllocationModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CashAllocationModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($cashAllocationId: ID!) {
						audits: cashAllocationAuditsByEntityId(cashAllocationId: $cashAllocationId) {
							entity {
								...CashAllocationProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<CashAllocationModel>> {
		const url = environment.API_URL + '/api/cashallocation/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<CashAllocationModel>> {
		const url = environment.API_URL + '/api/cashallocation/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<CashAllocationModel>> {
		let apiUrl = environment.API_URL + '/api/cashallocation/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<CashAllocationModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/cashallocation/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Cash Allocation entities')),
