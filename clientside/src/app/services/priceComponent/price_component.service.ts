...
import {PriceComponentModel} from '../../models/priceComponent/price_component.model';
import {PriceComponentModelAudit} from '../../models/priceComponent/price_component.model.state';
...
/**
 * Price Component service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class PriceComponentService extends AbstractHttpService<PriceComponentModel, PriceComponentModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the PriceComponentProperties classFragment here] off begin
			fragment PriceComponentProperties on PriceComponent {
				...PriceComponentBaseProperties
				componentCode
				componentName
				coaCOGS
				coaSell
			...
			# % protected region % [Modify the PriceComponentProperties classFragment here] end
			${super.fragments('PriceComponent').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		return  this.apollo.query<PassableStateConfig<PriceComponentModel>>(
			{
				query: gql`
					query Count {
						totalCount: countPriceComponents
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<PriceComponentModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		return this.apollo.query<PassableStateConfig<PriceComponentModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						PriceComponentById: priceComponentById(id: $id) {
							...PriceComponentProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.PriceComponentById as PriceComponentModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		return this.apollo.query<PassableStateConfig<PriceComponentModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						priceComponents: priceComponents(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...PriceComponentProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countPriceComponents(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.priceComponents as PriceComponentModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		return this.apollo.query<PassableStateConfig<PriceComponentModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						priceComponents: lastPriceComponents(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...PriceComponentProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: PriceComponentModel, queryParams?: QueryParams): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		const successMessage = 'Created Price Component entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($priceComponent: PriceComponentInput!, $persistRelations: Boolean) {
						priceComponent: createPriceComponent(priceComponent: $priceComponent, persistRelations: $persistRelations) {
							...PriceComponentProperties,
				...
				variables: {
					priceComponent: {
						...entity.toJSON(),
						invoiceItemComponents: entity.invoiceItemComponents ? entity.invoiceItemComponents?.map((model: InvoiceItemComponentModel) => new InvoiceItemComponentModel(model).toJSON()) : [],
						medicalFeeItemComponents: entity.medicalFeeItemComponents ? entity.medicalFeeItemComponents?.map((model: MedicalFeeItemComponentModel) => new MedicalFeeItemComponentModel(model).toJSON()) : [],
						tariffDefinitionOtherComponents: entity.tariffDefinitionOtherComponents ? entity.tariffDefinitionOtherComponents?.map((model: TariffDefinitionOtherComponentModel) => new TariffDefinitionOtherComponentModel(model).toJSON()) : [],
					},
			...
			map(result => {
				...
				return {
					targetModel: result.data.priceComponent as PriceComponentModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: PriceComponentModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		const successMessage = 'Created Multiple Price Component entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($priceComponents: [PriceComponentInput!]!) {
						priceComponent: createAllPriceComponent(priceComponents: $priceComponents) {
							...PriceComponentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					priceComponents: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.priceComponent as PriceComponentModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: PriceComponentModel, queryParams?: QueryParams): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		const successMessage = 'Updated Price Component entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($priceComponent: PriceComponentInput!, $persistRelations: Boolean) {
						priceComponent: updatePriceComponent(priceComponent: $priceComponent, persistRelations: $persistRelations) {
							...PriceComponentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: PriceComponentModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		const successMessage = 'Update Multiple Price Component entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($priceComponents: [PriceComponentInput!]!) {
						priceComponent: updateAllPriceComponent(priceComponents: $priceComponents) {
							...PriceComponentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		const successMessage = 'Deleted Price Component entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($priceComponentId: ID!) {
						priceComponentId: deletePriceComponentById(priceComponentId: $priceComponentId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.priceComponentId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		const successMessage = 'Deleted selected Price Component entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($priceComponentIds: [ID!]!){
						deletePriceComponentExcludingIds(priceComponentIds: $priceComponentIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.priceComponentIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<PriceComponentModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($priceComponentIds: [ID!]!) {
						priceComponentIds: deleteAllPriceComponentByIds(priceComponentIds: $priceComponentIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<PriceComponentModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PriceComponentModel>>(
			...
			map(result => {
				let audits: PriceComponentModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new PriceComponentModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<PriceComponentModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PriceComponentModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($priceComponentId: ID!) {
						audits: priceComponentAuditsByEntityId(priceComponentId: $priceComponentId) {
							entity {
								...PriceComponentProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<PriceComponentModel>> {
		const url = environment.API_URL + '/api/pricecomponent/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<PriceComponentModel>> {
		const url = environment.API_URL + '/api/pricecomponent/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<PriceComponentModel>> {
		let apiUrl = environment.API_URL + '/api/pricecomponent/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<PriceComponentModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/pricecomponent/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Price Component entities')),
