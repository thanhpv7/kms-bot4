...
import {PrescriptionItemModel} from '../../models/prescriptionItem/prescription_item.model';
import {PrescriptionItemModelAudit} from '../../models/prescriptionItem/prescription_item.model.state';
...
/**
 * Prescription Item service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class PrescriptionItemService extends AbstractHttpService<PrescriptionItemModel, PrescriptionItemModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the PrescriptionItemProperties classFragment here] off begin
			fragment PrescriptionItemProperties on PrescriptionItem {
				...PrescriptionItemBaseProperties
				iterationStart
				iterationEnd
				requestedQuantity
				allocatedQuantity
			...
			# % protected region % [Modify the PrescriptionItemProperties classFragment here] end
			${super.fragments('PrescriptionItem').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		return  this.apollo.query<PassableStateConfig<PrescriptionItemModel>>(
			{
				query: gql`
					query Count {
						totalCount: countPrescriptionItems
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<PrescriptionItemModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		return this.apollo.query<PassableStateConfig<PrescriptionItemModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						PrescriptionItemById: prescriptionItemById(id: $id) {
							...PrescriptionItemProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.PrescriptionItemById as PrescriptionItemModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		return this.apollo.query<PassableStateConfig<PrescriptionItemModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						prescriptionItems: prescriptionItems(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...PrescriptionItemProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countPrescriptionItems(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.prescriptionItems as PrescriptionItemModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		return this.apollo.query<PassableStateConfig<PrescriptionItemModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						prescriptionItems: lastPrescriptionItems(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...PrescriptionItemProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: PrescriptionItemModel, queryParams?: QueryParams): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		const successMessage = 'Created Prescription Item entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($prescriptionItem: PrescriptionItemInput!, $persistRelations: Boolean) {
						prescriptionItem: createPrescriptionItem(prescriptionItem: $prescriptionItem, persistRelations: $persistRelations) {
							...PrescriptionItemProperties,
				...
				variables: {
					prescriptionItem: {
						...entity.toJSON(),
						compound: entity.compound ? new PrescriptionCompoundModel(entity.compound).toJSON() : undefined,
						stockCatalogueId: entity.stockCatalogueId ? new StockCatalogueModel(entity.stockCatalogueId).toJSON() : undefined,
						prescription: entity.prescription ? new PrescriptionHeaderModel(entity.prescription).toJSON() : undefined,
			...
			map(result => {
				...
				return {
					targetModel: result.data.prescriptionItem as PrescriptionItemModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: PrescriptionItemModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		const successMessage = 'Created Multiple Prescription Item entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($prescriptionItems: [PrescriptionItemInput!]!) {
						prescriptionItem: createAllPrescriptionItem(prescriptionItems: $prescriptionItems) {
							...PrescriptionItemProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					prescriptionItems: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.prescriptionItem as PrescriptionItemModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: PrescriptionItemModel, queryParams?: QueryParams): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		const successMessage = 'Updated Prescription Item entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($prescriptionItem: PrescriptionItemInput!, $persistRelations: Boolean) {
						prescriptionItem: updatePrescriptionItem(prescriptionItem: $prescriptionItem, persistRelations: $persistRelations) {
							...PrescriptionItemProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: PrescriptionItemModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		const successMessage = 'Update Multiple Prescription Item entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($prescriptionItems: [PrescriptionItemInput!]!) {
						prescriptionItem: updateAllPrescriptionItem(prescriptionItems: $prescriptionItems) {
							...PrescriptionItemProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		const successMessage = 'Deleted Prescription Item entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($prescriptionItemId: ID!) {
						prescriptionItemId: deletePrescriptionItemById(prescriptionItemId: $prescriptionItemId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.prescriptionItemId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		const successMessage = 'Deleted selected Prescription Item entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($prescriptionItemIds: [ID!]!){
						deletePrescriptionItemExcludingIds(prescriptionItemIds: $prescriptionItemIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.prescriptionItemIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<PrescriptionItemModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($prescriptionItemIds: [ID!]!) {
						prescriptionItemIds: deleteAllPrescriptionItemByIds(prescriptionItemIds: $prescriptionItemIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<PrescriptionItemModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PrescriptionItemModel>>(
			...
			map(result => {
				let audits: PrescriptionItemModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new PrescriptionItemModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<PrescriptionItemModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PrescriptionItemModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($prescriptionItemId: ID!) {
						audits: prescriptionItemAuditsByEntityId(prescriptionItemId: $prescriptionItemId) {
							entity {
								...PrescriptionItemProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<PrescriptionItemModel>> {
		const url = environment.API_URL + '/api/prescriptionitem/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<PrescriptionItemModel>> {
		const url = environment.API_URL + '/api/prescriptionitem/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<PrescriptionItemModel>> {
		let apiUrl = environment.API_URL + '/api/prescriptionitem/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<PrescriptionItemModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/prescriptionitem/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Prescription Item entities')),
