...
import {CatalogueUOMModel} from '../../models/catalogueUOM/catalogue_uom.model';
import {CatalogueUOMModelAudit} from '../../models/catalogueUOM/catalogue_uom.model.state';
...
/**
 * Catalogue UOM service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class CatalogueUOMService extends AbstractHttpService<CatalogueUOMModel, CatalogueUOMModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the CatalogueUOMProperties classFragment here] off begin
			fragment CatalogueUOMProperties on CatalogueUOM {
				...CatalogueUOMBaseProperties
				unitName
				conversion
				defaultTransaction
				defaultStockCard
			...
			# % protected region % [Modify the CatalogueUOMProperties classFragment here] end
			${super.fragments('CatalogueUOM').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		return  this.apollo.query<PassableStateConfig<CatalogueUOMModel>>(
			{
				query: gql`
					query Count {
						totalCount: countCatalogueUOMs
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<CatalogueUOMModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		return this.apollo.query<PassableStateConfig<CatalogueUOMModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						CatalogueUOMById: catalogueUOMById(id: $id) {
							...CatalogueUOMProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.CatalogueUOMById as CatalogueUOMModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		return this.apollo.query<PassableStateConfig<CatalogueUOMModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						catalogueUOMs: catalogueUOMs(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...CatalogueUOMProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countCatalogueUOMs(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.catalogueUOMs as CatalogueUOMModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		return this.apollo.query<PassableStateConfig<CatalogueUOMModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						catalogueUOMs: lastCatalogueUOMs(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...CatalogueUOMProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: CatalogueUOMModel, queryParams?: QueryParams): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		const successMessage = 'Created Catalogue UOM entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($catalogueUOM: CatalogueUOMInput!, $persistRelations: Boolean) {
						catalogueUOM: createCatalogueUOM(catalogueUOM: $catalogueUOM, persistRelations: $persistRelations) {
							...CatalogueUOMProperties,
				...
				variables: {
					catalogueUOM: {
						...entity.toJSON(),
						unitOfMeasurement: entity.unitOfMeasurement ? new UnitOfMeasurementModel(entity.unitOfMeasurement).toJSON() : undefined,
			...
			map(result => {
				...
				return {
					targetModel: result.data.catalogueUOM as CatalogueUOMModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: CatalogueUOMModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		const successMessage = 'Created Multiple Catalogue UOM entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($catalogueUOMs: [CatalogueUOMInput!]!) {
						catalogueUOM: createAllCatalogueUOM(catalogueUOMs: $catalogueUOMs) {
							...CatalogueUOMProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					catalogueUOMs: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.catalogueUOM as CatalogueUOMModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: CatalogueUOMModel, queryParams?: QueryParams): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		const successMessage = 'Updated Catalogue UOM entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($catalogueUOM: CatalogueUOMInput!, $persistRelations: Boolean) {
						catalogueUOM: updateCatalogueUOM(catalogueUOM: $catalogueUOM, persistRelations: $persistRelations) {
							...CatalogueUOMProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: CatalogueUOMModel[], queryParams?: QueryParams): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		const successMessage = 'Update Multiple Catalogue UOM entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($catalogueUOMs: [CatalogueUOMInput!]!) {
						catalogueUOM: updateAllCatalogueUOM(catalogueUOMs: $catalogueUOMs) {
							...CatalogueUOMProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		const successMessage = 'Deleted Catalogue UOM entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($catalogueUOMId: ID!) {
						catalogueUOMId: deleteCatalogueUOMById(catalogueUOMId: $catalogueUOMId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.catalogueUOMId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		const successMessage = 'Deleted selected Catalogue UOM entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($catalogueUOMIds: [ID!]!){
						deleteCatalogueUOMExcludingIds(catalogueUOMIds: $catalogueUOMIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.catalogueUOMIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<CatalogueUOMModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($catalogueUOMIds: [ID!]!) {
						catalogueUOMIds: deleteAllCatalogueUOMByIds(catalogueUOMIds: $catalogueUOMIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<CatalogueUOMModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CatalogueUOMModel>>(
			...
			map(result => {
				let audits: CatalogueUOMModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new CatalogueUOMModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<CatalogueUOMModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<CatalogueUOMModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($catalogueUOMId: ID!) {
						audits: catalogueUOMAuditsByEntityId(catalogueUOMId: $catalogueUOMId) {
							entity {
								...CatalogueUOMProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<CatalogueUOMModel>> {
		const url = environment.API_URL + '/api/catalogueuom/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<CatalogueUOMModel>> {
		const url = environment.API_URL + '/api/catalogueuom/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<CatalogueUOMModel>> {
		let apiUrl = environment.API_URL + '/api/catalogueuom/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<CatalogueUOMModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/catalogueuom/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Catalogue UOM entities')),
