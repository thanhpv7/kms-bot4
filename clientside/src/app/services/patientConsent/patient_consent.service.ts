...
import {PatientConsentModelAudit} from '../../models/patientConsent/patient_consent.model.state';
...
/**
 * Patient Consent service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class PatientConsentService extends AbstractHttpService<PatientConsentModel, PatientConsentModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the PatientConsentProperties classFragment here] off begin
			fragment PatientConsentProperties on PatientConsent {
				...PatientConsentBaseProperties
				consentDateTime
				consentOnPaymentTerms
				consentOnRightsAndObligations
				consentOnHospitalRegulations
				needInterpreter
				needReligiousCounselor
				dgsResultsReleasedToInsurer
				dgsResultsReleasedToStudents
				recordReleasedToFamilyMember
				recordReleasedToCertainHealthFacilities
				staffSignature
				consentingPartySignature
				consentingPartyType
				consentingPartyName
				readAndSignSatusehat
				informationReleasedToSatusehat
				emrReleasedToSatusehat
			}
			# % protected region % [Modify the PatientConsentProperties classFragment here] end
			${super.fragments('PatientConsent').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		return  this.apollo.query<PassableStateConfig<PatientConsentModel>>(
			{
				query: gql`
					query Count {
						totalCount: countPatientConsents
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<PatientConsentModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		return this.apollo.query<PassableStateConfig<PatientConsentModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						PatientConsentById: patientConsentById(id: $id) {
							...PatientConsentProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.PatientConsentById as PatientConsentModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		return this.apollo.query<PassableStateConfig<PatientConsentModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						patientConsents: patientConsents(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...PatientConsentProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countPatientConsents(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.patientConsents as PatientConsentModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		return this.apollo.query<PassableStateConfig<PatientConsentModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						patientConsents: lastPatientConsents(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...PatientConsentProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: PatientConsentModel, queryParams?: QueryParams): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		const successMessage = 'Created Patient Consent entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($patientConsent: PatientConsentInput!, $persistRelations: Boolean) {
						patientConsent: createPatientConsent(patientConsent: $patientConsent, persistRelations: $persistRelations) {
							...PatientConsentProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.patientConsent as PatientConsentModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: PatientConsentModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		const successMessage = 'Created Multiple Patient Consent entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($patientConsents: [PatientConsentInput!]!) {
						patientConsent: createAllPatientConsent(patientConsents: $patientConsents) {
							...PatientConsentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					patientConsents: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.patientConsent as PatientConsentModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: PatientConsentModel, queryParams?: QueryParams): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		const successMessage = 'Updated Patient Consent entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($patientConsent: PatientConsentInput!, $persistRelations: Boolean) {
						patientConsent: updatePatientConsent(patientConsent: $patientConsent, persistRelations: $persistRelations) {
							...PatientConsentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: PatientConsentModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		const successMessage = 'Update Multiple Patient Consent entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($patientConsents: [PatientConsentInput!]!) {
						patientConsent: updateAllPatientConsent(patientConsents: $patientConsents) {
							...PatientConsentProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		const successMessage = 'Deleted Patient Consent entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($patientConsentId: ID!) {
						patientConsentId: deletePatientConsentById(patientConsentId: $patientConsentId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.patientConsentId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		const successMessage = 'Deleted selected Patient Consent entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($patientConsentIds: [ID!]!){
						deletePatientConsentExcludingIds(patientConsentIds: $patientConsentIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.patientConsentIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<PatientConsentModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($patientConsentIds: [ID!]!) {
						patientConsentIds: deleteAllPatientConsentByIds(patientConsentIds: $patientConsentIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<PatientConsentModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PatientConsentModel>>(
			...
			map(result => {
				let audits: PatientConsentModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new PatientConsentModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<PatientConsentModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PatientConsentModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($patientConsentId: ID!) {
						audits: patientConsentAuditsByEntityId(patientConsentId: $patientConsentId) {
							entity {
								...PatientConsentProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<PatientConsentModel>> {
		const url = environment.API_URL + '/api/patientconsent/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<PatientConsentModel>> {
		const url = environment.API_URL + '/api/patientconsent/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<PatientConsentModel>> {
		let apiUrl = environment.API_URL + '/api/patientconsent/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<PatientConsentModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/patientconsent/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Patient Consent entities')),
