...
import {DoctorUserModel} from '../../models/doctorUser/doctor_user.model';
import {DoctorUserModelAudit} from '../../models/doctorUser/doctor_user.model.state';
...
/**
 * Doctor User service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class DoctorUserService extends UserService<DoctorUserModel, DoctorUserModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the DoctorUserProperties classFragment here] off begin
			fragment DoctorUserProperties on DoctorUser {
				...DoctorUserUserProperties
			}
			# % protected region % [Modify the DoctorUserProperties classFragment here] end
			${super.fragments('DoctorUser').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		return  this.apollo.query<PassableStateConfig<DoctorUserModel>>(
			{
				query: gql`
					query Count {
						totalCount: countDoctorUsers
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<DoctorUserModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<DoctorUserModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						DoctorUserById: doctorUserById(id: $id) {
							...DoctorUserProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.DoctorUserById as DoctorUserModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<DoctorUserModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						doctorUsers: doctorUsers(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...DoctorUserProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countDoctorUsers(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.doctorUsers as DoctorUserModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		return this.apollo.query<PassableStateConfig<DoctorUserModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						doctorUsers: lastDoctorUsers(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...DoctorUserProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: DoctorUserModel, queryParams?: QueryParams): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		const successMessage = 'Created Doctor User entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($doctorUser: DoctorUserInput!, $persistRelations: Boolean) {
						doctorUser: createDoctorUser(doctorUser: $doctorUser, persistRelations: $persistRelations) {
							...DoctorUserProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.doctorUser as DoctorUserModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: DoctorUserModel[], queryParams?: QueryParams): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		const successMessage = 'Created Multiple Doctor User entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($doctorUsers: [DoctorUserInput!]!) {
						doctorUser: createAllDoctorUser(doctorUsers: $doctorUsers) {
							...DoctorUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					doctorUsers: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.doctorUser as DoctorUserModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: DoctorUserModel, queryParams?: QueryParams): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		const successMessage = 'Updated Doctor User entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($doctorUser: DoctorUserInput!, $persistRelations: Boolean) {
						doctorUser: updateDoctorUser(doctorUser: $doctorUser, persistRelations: $persistRelations) {
							...DoctorUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: DoctorUserModel[], queryParams?: QueryParams): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		const successMessage = 'Update Multiple Doctor User entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($doctorUsers: [DoctorUserInput!]!) {
						doctorUser: updateAllDoctorUser(doctorUsers: $doctorUsers) {
							...DoctorUserProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		const successMessage = 'Deleted Doctor User entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($doctorUserId: ID!) {
						doctorUserId: deleteDoctorUserById(doctorUserId: $doctorUserId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.doctorUserId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		const successMessage = 'Deleted selected Doctor User entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($doctorUserIds: [ID!]!){
						deleteDoctorUserExcludingIds(doctorUserIds: $doctorUserIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.doctorUserIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<DoctorUserModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($doctorUserIds: [ID!]!) {
						doctorUserIds: deleteAllDoctorUserByIds(doctorUserIds: $doctorUserIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<DoctorUserModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<DoctorUserModel>>(
			...
			map(result => {
				let audits: DoctorUserModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new DoctorUserModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<DoctorUserModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<DoctorUserModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($doctorUserId: ID!) {
						audits: doctorUserAuditsByEntityId(doctorUserId: $doctorUserId) {
							entity {
								...DoctorUserProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<DoctorUserModel>> {
		const url = environment.API_URL + '/api/doctoruser/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<DoctorUserModel>> {
		const url = environment.API_URL + '/api/doctoruser/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<DoctorUserModel>> {
		let apiUrl = environment.API_URL + '/api/doctoruser/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<DoctorUserModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/doctoruser/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Doctor User entities')),
