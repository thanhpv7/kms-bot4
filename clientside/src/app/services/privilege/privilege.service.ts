...
import {PrivilegeModelAudit} from '../../models/privilege/privilege.model.state';
...
/**
 * Privilege service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class PrivilegeService extends AbstractHttpService<PrivilegeModel, PrivilegeModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the PrivilegeProperties classFragment here] off begin
			fragment PrivilegeProperties on Privilege {
				...PrivilegeBaseProperties
				name
				targetEntity
				allowCreate
				allowRead
			...
			# % protected region % [Modify the PrivilegeProperties classFragment here] end
			${super.fragments('Privilege').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		return  this.apollo.query<PassableStateConfig<PrivilegeModel>>(
			{
				query: gql`
					query Count {
						totalCount: countPrivileges
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<PrivilegeModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		return this.apollo.query<PassableStateConfig<PrivilegeModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						PrivilegeById: privilegeById(id: $id) {
							...PrivilegeProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.PrivilegeById as PrivilegeModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		return this.apollo.query<PassableStateConfig<PrivilegeModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						privileges: privileges(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...PrivilegeProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countPrivileges(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.privileges as PrivilegeModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		return this.apollo.query<PassableStateConfig<PrivilegeModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						privileges: lastPrivileges(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...PrivilegeProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: PrivilegeModel, queryParams?: QueryParams): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		const successMessage = 'Created Privilege entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($privilege: PrivilegeInput!, $persistRelations: Boolean) {
						privilege: createPrivilege(privilege: $privilege, persistRelations: $persistRelations) {
							...PrivilegeProperties,
				...
				variables: {
					privilege: {
						...entity.toJSON(),
						roles: entity.roles ? entity.roles?.map((model: RoleModel) => new RoleModel(model).toJSON()) : [],
					},
			...
			map(result => {
				...
				return {
					targetModel: result.data.privilege as PrivilegeModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: PrivilegeModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		const successMessage = 'Created Multiple Privilege entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($privileges: [PrivilegeInput!]!) {
						privilege: createAllPrivilege(privileges: $privileges) {
							...PrivilegeProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					privileges: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.privilege as PrivilegeModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: PrivilegeModel, queryParams?: QueryParams): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		const successMessage = 'Updated Privilege entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($privilege: PrivilegeInput!, $persistRelations: Boolean) {
						privilege: updatePrivilege(privilege: $privilege, persistRelations: $persistRelations) {
							...PrivilegeProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: PrivilegeModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		const successMessage = 'Update Multiple Privilege entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($privileges: [PrivilegeInput!]!) {
						privilege: updateAllPrivilege(privileges: $privileges) {
							...PrivilegeProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		const successMessage = 'Deleted Privilege entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($privilegeId: ID!) {
						privilegeId: deletePrivilegeById(privilegeId: $privilegeId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.privilegeId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		const successMessage = 'Deleted selected Privilege entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($privilegeIds: [ID!]!){
						deletePrivilegeExcludingIds(privilegeIds: $privilegeIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.privilegeIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<PrivilegeModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($privilegeIds: [ID!]!) {
						privilegeIds: deleteAllPrivilegeByIds(privilegeIds: $privilegeIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<PrivilegeModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PrivilegeModel>>(
			...
			map(result => {
				let audits: PrivilegeModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new PrivilegeModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<PrivilegeModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PrivilegeModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($privilegeId: ID!) {
						audits: privilegeAuditsByEntityId(privilegeId: $privilegeId) {
							entity {
								...PrivilegeProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<PrivilegeModel>> {
		const url = environment.API_URL + '/api/privilege/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<PrivilegeModel>> {
		const url = environment.API_URL + '/api/privilege/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<PrivilegeModel>> {
		let apiUrl = environment.API_URL + '/api/privilege/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<PrivilegeModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/privilege/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Privilege entities')),
