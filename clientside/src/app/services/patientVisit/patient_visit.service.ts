...
import {PatientVisitModel} from '../../models/patientVisit/patient_visit.model';
import {PatientVisitModelAudit} from '../../models/patientVisit/patient_visit.model.state';
...
/**
 * Patient Visit service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class PatientVisitService extends AbstractHttpService<PatientVisitModel, PatientVisitModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the PatientVisitProperties classFragment here] off begin
			fragment PatientVisitProperties on PatientVisit {
				...PatientVisitBaseProperties
				visitNumber
				registrationType
				visitDate
			...
			# % protected region % [Modify the PatientVisitProperties classFragment here] end
			${super.fragments('PatientVisit').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		return  this.apollo.query<PassableStateConfig<PatientVisitModel>>(
			{
				query: gql`
					query Count {
						totalCount: countPatientVisits
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<PatientVisitModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		return this.apollo.query<PassableStateConfig<PatientVisitModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						PatientVisitById: patientVisitById(id: $id) {
							...PatientVisitProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.PatientVisitById as PatientVisitModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		return this.apollo.query<PassableStateConfig<PatientVisitModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						patientVisits: patientVisits(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...PatientVisitProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countPatientVisits(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.patientVisits as PatientVisitModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		return this.apollo.query<PassableStateConfig<PatientVisitModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						patientVisits: lastPatientVisits(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...PatientVisitProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: PatientVisitModel, queryParams?: QueryParams): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		const successMessage = 'Created Patient Visit entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($patientVisit: PatientVisitInput!, $persistRelations: Boolean) {
						patientVisit: createPatientVisit(patientVisit: $patientVisit, persistRelations: $persistRelations) {
							...PatientVisitProperties,
				...
				variables: {
					patientVisit: {
						...
						patientVisitPatientPaymentInsurance: entity.patientVisitPatientPaymentInsurance ? new PatientVisitPatientPaymentInsuranceModel(entity.patientVisitPatientPaymentInsurance).toJSON() : undefined,
						patientVisitPatientPaymentOthers: entity.patientVisitPatientPaymentOthers ? new PatientVisitPatientPaymentOthersModel(entity.patientVisitPatientPaymentOthers).toJSON() : undefined,
						patientVisitPaymentSelfPaying: entity.patientVisitPaymentSelfPaying ? new PatientVisitPaymentSelfPayingModel(entity.patientVisitPaymentSelfPaying).toJSON() : undefined,
						patient: entity.patient ? new PatientGeneralInfoModel(entity.patient).toJSON() : undefined,
						bpjsVisitDatas: entity.bpjsVisitDatas ? entity.bpjsVisitDatas?.map((model: BpjsVisitDataModel) => new BpjsVisitDataModel(model).toJSON()) : [],
						bpjsPcarePendaftarans: entity.bpjsPcarePendaftarans ? entity.bpjsPcarePendaftarans?.map((model: BpjsPcarePendaftaranModel) => new BpjsPcarePendaftaranModel(model).toJSON()) : [],
			...
			map(result => {
				...
				return {
					targetModel: result.data.patientVisit as PatientVisitModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: PatientVisitModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		const successMessage = 'Created Multiple Patient Visit entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($patientVisits: [PatientVisitInput!]!) {
						patientVisit: createAllPatientVisit(patientVisits: $patientVisits) {
							...PatientVisitProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					patientVisits: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.patientVisit as PatientVisitModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: PatientVisitModel, queryParams?: QueryParams): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		const successMessage = 'Updated Patient Visit entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($patientVisit: PatientVisitInput!, $persistRelations: Boolean) {
						patientVisit: updatePatientVisit(patientVisit: $patientVisit, persistRelations: $persistRelations) {
							...PatientVisitProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: PatientVisitModel[], queryParams?: QueryParams): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		const successMessage = 'Update Multiple Patient Visit entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($patientVisits: [PatientVisitInput!]!) {
						patientVisit: updateAllPatientVisit(patientVisits: $patientVisits) {
							...PatientVisitProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		const successMessage = 'Deleted Patient Visit entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($patientVisitId: ID!) {
						patientVisitId: deletePatientVisitById(patientVisitId: $patientVisitId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.patientVisitId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		const successMessage = 'Deleted selected Patient Visit entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($patientVisitIds: [ID!]!){
						deletePatientVisitExcludingIds(patientVisitIds: $patientVisitIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.patientVisitIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<PatientVisitModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($patientVisitIds: [ID!]!) {
						patientVisitIds: deleteAllPatientVisitByIds(patientVisitIds: $patientVisitIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<PatientVisitModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PatientVisitModel>>(
			...
			map(result => {
				let audits: PatientVisitModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new PatientVisitModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<PatientVisitModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<PatientVisitModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($patientVisitId: ID!) {
						audits: patientVisitAuditsByEntityId(patientVisitId: $patientVisitId) {
							entity {
								...PatientVisitProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<PatientVisitModel>> {
		const url = environment.API_URL + '/api/patientvisit/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<PatientVisitModel>> {
		const url = environment.API_URL + '/api/patientvisit/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<PatientVisitModel>> {
		let apiUrl = environment.API_URL + '/api/patientvisit/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<PatientVisitModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/patientvisit/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Patient Visit entities')),
