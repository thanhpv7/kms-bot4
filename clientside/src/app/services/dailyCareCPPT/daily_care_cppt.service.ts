...
import {DailyCareCPPTModel} from '../../models/dailyCareCPPT/daily_care_cppt.model';
import {DailyCareCPPTModelAudit} from '../../models/dailyCareCPPT/daily_care_cppt.model.state';
...
/**
 * Daily Care CPPT service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class DailyCareCPPTService extends AbstractHttpService<DailyCareCPPTModel, DailyCareCPPTModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the DailyCareCPPTProperties classFragment here] off begin
			fragment DailyCareCPPTProperties on DailyCareCPPT {
				...DailyCareCPPTBaseProperties
				subjective
				objective
				diagnose
				plan
			...
			# % protected region % [Modify the DailyCareCPPTProperties classFragment here] end
			${super.fragments('DailyCareCPPT').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		return  this.apollo.query<PassableStateConfig<DailyCareCPPTModel>>(
			{
				query: gql`
					query Count {
						totalCount: countDailyCareCPPTs
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<DailyCareCPPTModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		return this.apollo.query<PassableStateConfig<DailyCareCPPTModel>>(
			{
				query: gql`
					query Get($id: ID!) {
						DailyCareCPPTById: dailyCareCPPTById(id: $id) {
							...DailyCareCPPTProperties
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.DailyCareCPPTById as DailyCareCPPTModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		return this.apollo.query<PassableStateConfig<DailyCareCPPTModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						dailyCareCPPTs: dailyCareCPPTs(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...DailyCareCPPTProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countDailyCareCPPTs(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.dailyCareCPPTs as DailyCareCPPTModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		return this.apollo.query<PassableStateConfig<DailyCareCPPTModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						dailyCareCPPTs: lastDailyCareCPPTs(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...DailyCareCPPTProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: DailyCareCPPTModel, queryParams?: QueryParams): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		const successMessage = 'Created Daily Care CPPT entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($dailyCareCPPT: DailyCareCPPTInput!, $persistRelations: Boolean) {
						dailyCareCPPT: createDailyCareCPPT(dailyCareCPPT: $dailyCareCPPT, persistRelations: $persistRelations) {
							...DailyCareCPPTProperties,
				...
				variables: {
					dailyCareCPPT: {
						...entity.toJSON(),
						icd10: entity.icd10 ? new DiagnosesAndProceduresModel(entity.icd10).toJSON() : undefined,
						icd9CM: entity.icd9CM ? new DiagnosesAndProceduresModel(entity.icd9CM).toJSON() : undefined,
						createdByStaff: entity.createdByStaff ? new StaffModel(entity.createdByStaff).toJSON() : undefined,
						doctorInCharge: entity.doctorInCharge ? new StaffModel(entity.doctorInCharge).toJSON() : undefined,
			...
			map(result => {
				...
				return {
					targetModel: result.data.dailyCareCPPT as DailyCareCPPTModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: DailyCareCPPTModel[], queryParams?: QueryParams): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		const successMessage = 'Created Multiple Daily Care CPPT entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($dailyCareCPPTs: [DailyCareCPPTInput!]!) {
						dailyCareCPPT: createAllDailyCareCPPT(dailyCareCPPTs: $dailyCareCPPTs) {
							...DailyCareCPPTProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					dailyCareCPPTs: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.dailyCareCPPT as DailyCareCPPTModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: DailyCareCPPTModel, queryParams?: QueryParams): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		const successMessage = 'Updated Daily Care CPPT entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($dailyCareCPPT: DailyCareCPPTInput!, $persistRelations: Boolean) {
						dailyCareCPPT: updateDailyCareCPPT(dailyCareCPPT: $dailyCareCPPT, persistRelations: $persistRelations) {
							...DailyCareCPPTProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: DailyCareCPPTModel[], queryParams?: QueryParams): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		const successMessage = 'Update Multiple Daily Care CPPT entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($dailyCareCPPTs: [DailyCareCPPTInput!]!) {
						dailyCareCPPT: updateAllDailyCareCPPT(dailyCareCPPTs: $dailyCareCPPTs) {
							...DailyCareCPPTProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		const successMessage = 'Deleted Daily Care CPPT entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($dailyCareCPPTId: ID!) {
						dailyCareCPPTId: deleteDailyCareCPPTById(dailyCareCPPTId: $dailyCareCPPTId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.dailyCareCPPTId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		const successMessage = 'Deleted selected Daily Care CPPT entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($dailyCareCPPTIds: [ID!]!){
						deleteDailyCareCPPTExcludingIds(dailyCareCPPTIds: $dailyCareCPPTIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.dailyCareCPPTIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($dailyCareCPPTIds: [ID!]!) {
						dailyCareCPPTIds: deleteAllDailyCareCPPTByIds(dailyCareCPPTIds: $dailyCareCPPTIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<DailyCareCPPTModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<DailyCareCPPTModel>>(
			...
			map(result => {
				let audits: DailyCareCPPTModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new DailyCareCPPTModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<DailyCareCPPTModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<DailyCareCPPTModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($dailyCareCPPTId: ID!) {
						audits: dailyCareCPPTAuditsByEntityId(dailyCareCPPTId: $dailyCareCPPTId) {
							entity {
								...DailyCareCPPTProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		const url = environment.API_URL + '/api/dailycarecppt/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		const url = environment.API_URL + '/api/dailycarecppt/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		let apiUrl = environment.API_URL + '/api/dailycarecppt/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<DailyCareCPPTModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/dailycarecppt/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' Daily Care CPPT entities')),
