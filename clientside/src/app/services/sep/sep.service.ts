...
import {SepModel} from '../../models/sep/sep.model';
import {SepModelAudit} from '../../models/sep/sep.model.state';
...
/**
 * SEP service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class SepService extends AbstractHttpService<SepModel, SepModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			# % protected region % [Modify the SepProperties classFragment here] off begin
			fragment SepProperties on Sep {
				...SepBaseProperties
				sepData
			}
			# % protected region % [Modify the SepProperties classFragment here] end
			${super.fragments('Sep').properties}
		`,
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<SepModel>> {
		...
		return  this.apollo.query<PassableStateConfig<SepModel>>(
			{
				query: gql`
					query Count {
						totalCount: countSeps
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<SepModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<SepModel>> {
		...
		return this.apollo.query<PassableStateConfig<SepModel>>(
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.SepById as SepModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<SepModel>> {
		...
		return this.apollo.query<PassableStateConfig<SepModel>>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						seps: seps(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...SepProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countSeps(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.seps as SepModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<SepModel>> {
		...
		return this.apollo.query<PassableStateConfig<SepModel>>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						seps: lastSeps(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...SepProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: SepModel, queryParams?: QueryParams): Observable<PassableStateConfig<SepModel>> {
		...
		const successMessage = 'Created SEP entity successfully';
		// % protected region % [Customise the success message for create here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($sep: SepInput!, $persistRelations: Boolean) {
						sep: createSep(sep: $sep, persistRelations: $persistRelations) {
							...SepProperties,
			...
			map(result => {
				...
				return {
					targetModel: result.data.sep as SepModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: SepModel[], queryParams?: QueryParams): Observable<PassableStateConfig<SepModel>> {
		...
		const successMessage = 'Created Multiple SEP entities successfully';
		// % protected region % [Customise the success message for createAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($seps: [SepInput!]!) {
						sep: createAllSep(seps: $seps) {
							...SepProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
				...
				`,
				variables: {
					seps: entities.map(entity => entity.toJSON()),
			...
			map(result => {
				...
				return {
					targetModels: result.data.sep as SepModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: SepModel, queryParams?: QueryParams): Observable<PassableStateConfig<SepModel>> {
		...
		const successMessage = 'Updated SEP entity successfully';
		// % protected region % [Customise the success message for update here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($sep: SepInput!, $persistRelations: Boolean) {
						sep: updateSep(sep: $sep, persistRelations: $persistRelations) {
							...SepProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: SepModel[], queryParams?: QueryParams): Observable<PassableStateConfig<SepModel>> {
		...
		const successMessage = 'Update Multiple SEP entities successfully';
		// % protected region % [Customise the success message for updateAll here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($seps: [SepInput!]!) {
						sep: updateAllSep(seps: $seps) {
							...SepProperties,
							${this.getExpands(queryParams ? queryParams.expands : [])}
						}
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<SepModel>> {
		...
		const successMessage = 'Deleted SEP entity successfully';
		// % protected region % [Customise the success message for delete here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($sepId: ID!) {
						sepId: deleteSepById(sepId: $sepId)
			...
			map(result => {
				...
				return {
					targetModelId: result.data.sepId as string,
	...
	deleteExcludingIds(ids: string[]): Observable<PassableStateConfig<SepModel>> {
		...
		const successMessage = 'Deleted selected SEP entities successfully';
		// % protected region % [Customise the success message for delete excluding here] end

		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation deleteAll($sepIds: [ID!]!){
						deleteSepExcludingIds(sepIds: $sepIds)
			...
			map(result => {
				...
				return {
					targetModelIds: result.data.sepIds as string[],
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<SepModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($sepIds: [ID!]!) {
						sepIds: deleteAllSepByIds(sepIds: $sepIds)
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<SepModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<SepModel>>(
			...
			map(result => {
				let audits: SepModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new SepModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<SepModelAudit[]> {
		...
		return this.apollo.query<PassableStateConfig<SepModel>>(
			{
				query: gql`
					query GetAuditsByEntityId($sepId: ID!) {
						audits: sepAuditsByEntityId(sepId: $sepId) {
							entity {
								...SepProperties
							}
							timestamp
	 ...
	 * @inheritdoc
	 */
	export(id: string): Observable<PassableStateConfig<SepModel>> {
		const url = environment.API_URL + '/api/sep/export';
	 ...
	 * @inheritdoc
	 */
	exportAll(ids: string[]): Observable<PassableStateConfig<SepModel>> {
		const url = environment.API_URL + '/api/sep/export';
	...
	exportModelsExcludingIds(ids: string[]): Observable<PassableStateConfig<SepModel>> {
		let apiUrl = environment.API_URL + '/api/sep/export-excluding?ids=';
	...
	importModels(file: File): Observable<PassableStateConfig<SepModel>> {
		const requestData: FormData = new FormData();
		requestData.append('file', file);

		let apiUrl = environment.API_URL + '/api/sep/import-csv';
		...
		return this.http.post(apiUrl, requestData, {withCredentials: true, responseType: 'text'}).pipe(
			tap(result => this.toastrService.success('Successfully Imported ' + result.split(',').length + ' SEP entities')),
